...
readonly RELEASE_HOST="target/release/${APP_HOST}"
...
if [[ ! -f "$RELEASE_HOST" ]]; then
    echo "error: ${RELEASE_HOST} not found after build step" >&2
    exit 1
fi
...
cp "$RELEASE_HOST" "${STAGE_DIR}/bin/${APP_HOST}"
chmod +x "${STAGE_DIR}/bin/${APP_HOST}"
...
install -m 755 bin/jerry-host "${BIN_DIR}/jerry-host"
echo "installed ${BIN_DIR}/jerry-host"
