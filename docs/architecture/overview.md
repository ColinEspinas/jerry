...
layer must never depend on an outer one. Concretely: **no crate below `crates/jerry-app` may ever gain a
`gpui` dependency.**

```
        ui (crates/jerry-app)              cli (crates/jerry-cli)
                \                        /
                 \                      /
                  v                    v
              application  — Commands, Queries, Ports (traits)
                       ...
                       domain  — pure types, zero I/O
                          ^
                          |
    adapters: git (jerry-git) · pty (jerry-pty) · lsp (jerry-lsp) · hooks
              implement the Ports; depend inward only
```

This already holds at the crate boundary: `jerry-git`, `jerry-pty`, and `jerry-lsp` have zero `gpui`
dependency today. The violations are all inside `crates/jerry-app` itself, where render code calls
...
`jerry-git` already exposes almost exactly this set as loose functions (`commit_all_changes`,
...
| Port | Adapter today |
|---|---|
| Git operations | `jerry-git` (via `gix` for reads, the real `git` CLI via explicit argv for writes) |
| Process spawning / PTY | `jerry-pty` (`portable-pty`) |
| Language server protocol | `jerry-lsp` |
| Agent hook side-channel | `crates/jerry-app/src/hooks/` (candidate extraction — see below) |

All three existing core crates are already synchronous, `Result`-returning, and blocking by design
— every public function in `jerry-git` is documented as blocking, on the assumption that a GPUI caller
...
`crates/jerry-app` currently holds ~24k lines of code with no `gpui` dependency at all, trapped inside the
UI crate because nothing forced it out. The largest and cleanest candidate is
`crates/jerry-app/src/hooks/` (~5,400 LOC — `server.rs`, `settings_file.rs`, `event.rs`, `store.rs`): a
...
1. **Replace `use super::*` globs with explicit imports** (284 occurrences in `crates/jerry-app`). Today a
...
2. **Remove adapter calls from `render.rs` files** — `graph_view/render.rs` alone has 109
   `jerry_git::` references, and `sidebar/render.rs:6534` shells out to
