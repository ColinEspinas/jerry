...
//! no error to debug.
//!
//! Writes go through a `Mutex` rather than a writer thread, unlike `jerry-pty`: callers are always
...
/// [`jerry_pty::new_std_command`],
/// through the same [`jerry_pty::resolve_on_path`] callers use to decide a server is installed.
///
...
fn resolve_server_binary(server: &'static str, binary: &'static str) -> Result<PathBuf, LspError> {
    resolve_server_binary_with(server, binary, jerry_pty::resolve_on_path)
}
...
impl LspClient {
    ...
    pub fn spawn(repo_root: &Path, config: ServerSpawnConfig) -> Result<Self, LspError> {
        ...
        // Through `jerry_pty::new_std_command`, so a `.cmd`-shim server (which `std` launches
        // via a console-subsystem `cmd.exe` host) opens no console window on Windows release
        // builds (GitHub issue #465).
        let mut command = jerry_pty::new_std_command(resolved_binary);
        command
            .args(&config.args)
    ...
    #[cfg(windows)]
    fn kill_process_tree(&mut self) {
        match jerry_pty::new_std_command("taskkill")
...
impl Drop for LspClient {
    fn drop(&mut self) {
        if !self.exited {
            ...
            #[cfg(windows)]
            {
                let _ = jerry_pty::new_std_command("taskkill")
                    .args(["/T", "/F", "/PID", &self.pid.to_string()])
                    .stdin(Stdio::null())
                    .stdout(Stdio::null())
                    .stderr(Stdio::null())
    ...
    loop {
        match transport::read_message(&mut reader) {
            ...
            Err(err) => {
                ...
                log::warn!("jerry-lsp reader thread stopping after a real I/O error: {err}");
                break;
            }
        }
    }
...
mod tests {
    ...
    /// The rust-analyzer config these tests spawn against, kept local since `jerry-lsp` must
    /// stand alone.
    ...
    {
        let dir = tempfile::TempDir::new().expect("tempdir");
        let script = dir.path().join("jerry_lsp_fake_server.cmd");
        ...
        let bare_name_result = Command::new("jerry_lsp_fake_server").output();
    ...
    /// Both paths send the same real wake signal, so the `jerry-app` crate's single existing poll loop
    ...
    /// supports_diagnostic_pull`'s own docs). An earlier version of this test (and of the `jerry-app`
