...
//!
//! Parallel to `jerry-pty`'s kill path rather than shared with it: that child is a process-group
...
/// requirement `jerry-pty::collect_descendant_pids`'s own docs describe).
#[cfg(unix)]
