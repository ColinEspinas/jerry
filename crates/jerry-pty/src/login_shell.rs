...
//!
//! `crates/jerry-app/src/main.rs` is the only intended caller: it merges the result into the real
...
mod resolve_login_shell_path_tests {
    ...
    /// `cargo test -p jerry-pty --lib -- --ignored resolves_a_real_homebrew_style_path`,
...
mod run_login_shell_probe_tests {
    ...
    fn captures_stdout_of_a_quick_real_command() {
        ...
        assert!(
            text.contains("probe-marker-jerry-pty"),
