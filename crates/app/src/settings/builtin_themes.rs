...
pub fn generate_builtin_theme_toml(source: &BuiltinThemeSource) -> String {
    ...
    let header = if is_jerry_dark {
        "# Jerry Dark - this app's default theme, and the base every other theme inherits from.\n\
         ...
         # compiled-in default of the matching `crate::theme` token, so restating them here would\n\
         # just be a second copy to keep in sync. `preview` is the five swatches its Themes-page\n\
         # card paints.\n\
         #\n\
        ...
        format!(
            "# {} - a bundled Jerry theme.\n\
             ...
             # swatches ({}), and written out literally - so it renders exactly\n\
             # as this theme always has, while now being an ordinary, hand-editable theme file with\n\
             # no special status. Edit any line freely; delete one and that token falls back to\n\
             # Jerry Dark's own value.\n\
             #\n\
    ...
    format!("{header}{}", theme_file_toml(&file))
}

/// [`CustomThemeFile`]'s own real writer, reached through a validated [`CustomTheme`] so a
/// generated file goes through the exact same serialization a user-exported one does.
fn theme_file_toml(file: &CustomThemeFile) -> String {
