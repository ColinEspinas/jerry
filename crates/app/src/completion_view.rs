...
//! the real byte range/text an [`crate::edit_buffer::EditBuffer`] mutation should splice in if
//! accepted. Deliberately `gpui`-free (no `gpui` type appears anywhere in this module), mirroring
//! `crate::hover_view`'s own split between pure logic here and `crate::root`'s live request
//! dispatch/popover painting - see that module's own top doc comment for the same convention.
//!
//! `crate::hover_view`'s own top docs used to say a Completions popup was out of scope because
...
//! `crate::root::completions`, its GPUI-facing counterpart) now build on.
...
/// start of the buffer. `crate::root::lsp::AdeApp::prepare_lsp_sync`'s own real trigger point:
...
/// one real Tab/Enter accept path - see `crate::root::completions`'s own docs), and `.insert` is
...
/// the fallback replace-range `crate::root::completions` uses for a completion item with no
...
/// `gpui`/buffer-free module - see `crate::root::completions::resolve_completion_edit`.
