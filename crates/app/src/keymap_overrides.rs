...
/// #242 phase B - `crate::graph_view::rebase_render`'s interactive-rebase plan row reword field),
/// which all emit the same bare `"text-input"`, and `crate::sidebar::render::AdeApp::
/// file_tree_shell` (one call site, four literals from a `match`) - and guarded against drift by
/// this module's own `every_real_key_context_call_site_is_covered` test, which fails the moment a
/// fourteenth call site appears. (That test earned its keep immediately: this comment originally
/// said "six", counting distinct emitted literals rather than real call sites, and the test
/// caught it on its first run; it has since caught this comment drifting behind three more real
/// inputs.)
///
    ...
    fn every_real_key_context_call_site_is_covered() {
        ...
        assert_eq!(
            ...
            "real_context_stacks() is hand-derived from exactly thirteen .key_context(..) call \
             ...
             field (GitHub issue #213), and - newest, GitHub issue #242 phase B - the \
