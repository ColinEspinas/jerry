...
//! and "discard worktree" (`wt_core::undo::discard_worktree`). Mirrors `crate::palette`/
//! `crate::work_surface`'s own split: this module only holds already-computed outcomes/snapshots
...
//! result here) happens one layer up, in `crate::root::worktree_history`, which owns the
...
//! `crate::root::worktree_history`'s own docs for the async completion-handler discipline this
...
impl UndoableAction {
    ...
    /// `secondary` line - see `crate::palette::HistoryCandidate`'s own docs) distinct from
...
impl UndoStack {
    ...
    /// describe real, current content) - see `crate::root::worktree_history::AdeApp::
