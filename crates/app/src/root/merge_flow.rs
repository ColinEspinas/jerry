...
impl AdeApp {
    ...
    pub(super) fn clear_merge_edit_state(&mut self) {
    ...
    pub(super) fn sync_merge_edit_to_active_file(&mut self) {
    ...
    /// `crate::root::merge_flow_render`'s own docs for the button this backs) - seeds a fresh
    /// [`crate::edit_buffer::EditBuffer`] from [`wt_core::merge::ConflictedFile::render`]'s
    ...
    /// see `crate::root::merge_editing`'s own top docs for why this surface never runs the real
    ...
    pub(super) fn start_merge_hand_edit(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let Some(flow) = self.merge_flow.as_ref() else {
            return;
        };
        let merge::MergeFlowState::Conflicted {
    ...
    /// `crate::edit_buffer`'s own documented "no undo/redo" scope cut for why this is a coarse
    /// whole-buffer discard, not a step-by-step one.
    pub(super) fn discard_merge_hand_edit(&mut self, cx: &mut Context<Self>) {
        self.clear_merge_edit_state();
        cx.notify();
    }
    ...
    /// matching `crate::merge::replace_conflicted_file` swap into `files[]`, a
    /// `crate::merge::first_unresolved` recompute, and [`Self::ensure_active_merge_highlight_cache`]
    ...
    pub(super) fn save_merge_edit(&mut self, cx: &mut Context<Self>) {
        let Some(edit) = self.merge_edit.as_ref() else {
            return;
        };
    ...
    fn spawn_merge_edit_save_loop(&mut self, cx: &mut Context<Self>) {
        let task = cx.spawn(async move |this, cx| {
            loop {
                ...
                // for it - see `crate::root::merge_flow_render`'s own docs) until either a clean
    ...
    /// owning [`merge::MergeFlow`]'s `files[]` (via `crate::merge::replace_conflicted_file`,
    /// matched by path), recomputes `active_file`/`active_hunk`
    /// ([`crate::merge::first_unresolved`]), and refreshes the highlight cache - the real
...
mod merge_regression_tests {
    ...
    /// Real, bound keystroke bindings - see `crate::root::editing::editing_tests::
    ...
    /// platform text-input layer itself calls - matching `crate::root::editing::editing_tests`'
    ...
    fn merge_editor_context_does_not_swallow_keystrokes_meant_for_a_different_focused_surface(
        ...
        // `crate::root::work_surface_render::AdeApp::render_center_pane`'s own visibility rule
        ...
        // a real, live bug in - `crate::root::editing::AdeApp::active_edit_target`'s docs): a
        ...
        // `crate::root::merge_editing`'s own row click handler makes) - simulated directly here
        // for the same reason `crate::root::code_surface`'s own click handlers are the real
    ...
    /// `crate::root::editing::AdeApp::active_edit_target` returned `None` whenever
    /// `AdeApp::open_change` was `Some`, but `crate::root::work_surface_render::AdeApp::
    ...
    /// is still `Some`: `crate::root::code_surface::AdeApp::refresh_open_diff_file_cache`
    ...
    /// `crate::root::code_surface::AdeApp::activate_file_tab`'s own doc comment describes ("the
    ...
    /// here (the same established precedent `root::status_bar`'s own tests already use for this
