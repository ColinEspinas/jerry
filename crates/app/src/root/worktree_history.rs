...
//! (`crate::root::rail_render`). This is a deliberate simplification of this project's usual
...
pub(super) enum WorktreeHistoryOpKind {
    Keep,
    Discard,
    Undo,
    Redo,
...
impl AdeApp {
    ...
    /// `Undo`.
    pub(super) fn keep_all_changes(&mut self, id: SessionId, cx: &mut Context<Self>) {
    ...
    pub(super) fn request_discard_worktree(
        &mut self,
        id: SessionId,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn execute_discard_worktree(
        &mut self,
        id: SessionId,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn handle_undo_action(
        &mut self,
        _action: &Undo,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn handle_redo_action(
        &mut self,
        _action: &Redo,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ...
    /// module's own docs.
    pub(super) fn perform_undo(&mut self, cx: &mut Context<Self>) {
        if self.worktree_history_op_in_flight.is_some() {
    ...
    /// snapshot.
    pub(super) fn perform_redo(&mut self, cx: &mut Context<Self>) {
        if self.worktree_history_op_in_flight.is_some() {
...
/// `root::merge_flow::merge_regression_tests`/`root::rail_render::prune_regression_tests`' own
/// established idiom.
#[cfg(test)]
mod worktree_history_regression_tests {
    ...
    /// Same linked-worktree idiom `root::merge_flow`/`root::rail_render`'s own test modules use.
