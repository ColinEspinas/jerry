...
//! of the read-only two-column quick-pick view (`crate::root::merge_flow_render::
...
//!
//! The real editing engine ([`crate::edit_buffer::EditBuffer`]), the single, generalized
//! `EntityInputHandler` impl, and the `Editor*` action handler *bodies* (`crate::root::editing`)
//! are all reused as-is, routed here whenever [`crate::root::editing::AdeApp::active_edit_target`]
...
//! equivalents (`crate::root::editing::render_editable_file_view_line`,
...
//!
//! [`AdeApp::start_merge_hand_edit`] seeds this view's [`crate::edit_buffer::EditBuffer`] with
//! `extension: None`, so [`crate::edit_buffer::EditBuffer::highlighter`] always resolves to
...
//! `crate::root::editing::AdeApp::schedule_rehighlight`), no diagnostics gutter, no hover, and no
...
use super::editing::split_runs_for_marked_range;
use super::*;
...
impl AdeApp {
    ...
    /// file (see `crate::root::merge_flow_render`'s own docs for exactly where this is called
    /// from) - `&self`, matching `crate::root::merge_flow_render::AdeApp::
    ...
    /// render call).
    pub(super) fn render_merge_edit_view(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let code = uniform_list(
            ...
            cx.processor(move |this: &mut Self, range: Range<usize>, _window, cx| {
                ...
                // `crate::root::code_surface::AdeApp::render_file_view`'s identical discipline
        ...
        div()
            ...
            // discipline `crate::root::code_surface::AdeApp::render_code_surface` already
...
/// smaller equivalent of `crate::root::editing::EditableLineContext` (no diagnostics, no hover -
...
/// `crate::root::editing::render_editable_file_view_line`'s real per-row div-based text plus
...
/// exact same real function the File view's own `crate::root::editing::build_text_runs` uses for
...
/// `crate::root::editing::build_text_run_divs`'s identical real splitting (minus the
