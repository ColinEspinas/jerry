...
impl AdeApp {
    ...
    ///
    /// Linux-only (`#[cfg(target_os = "linux")]`, matching `crate::process_stats`'s own scope) -
    ...
    /// Non-Linux build: `crate::process_stats` has no real sampling to offer on this platform
    ...
    /// `Self::render_rail_footer` shows, via `crate::rail::disk_usage_bytes`/
    ...
    /// `code_surface::render_file_status_bar` already computed and displayed for this same file
    ...
    fn render_status_zoom_value(&self, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            // `root::code_surface`'s own click-driven tests use.
...
/// own zoom control (`root::code_surface::render_zoom_control`) uses, never a second, copied
/// implementation.
#[cfg(test)]
...
/// the File view's own footer (`code_surface::render_file_status_bar`) - not once per line the
