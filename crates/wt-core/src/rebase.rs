...
mod tests {
    ...
    fn amend_head_message_refuses_when_head_has_moved_since_expected() {
        ...
        // Refused, not partially applied: the message must be untouched.
        let head = git_output(repo.path(), &["rev-parse", "HEAD"]);
    ...
    fn amend_head_message_refuses_when_the_real_index_has_staged_changes() {
        ...
        // A staged change unrelated to the amend: what the guard exists to catch.
    ...
    fn reword_with_no_message_stops_and_reports_the_right_commit_and_reason() {
        ...
        // Amend, then continue: completes with the amended message applied.
    ...
    /// A conflict-resumed `pick` invokes `GIT_EDITOR` too, through git's ordinary `commit`
    /// codepath. Misclassifying that as a `reword` consumes a queue slot meant for a later row,
    /// applying the wrong message or stopping when it should not.
    #[test]
