...
impl AdeApp {
    ...
    /// (`jerry_git::merge::abort_merge`) rather than leaving the repository mid-merge with no UI
    ...
    /// child process - see `jerry_git::merge::attempt_merge`'s own docs).
    #[allow(dead_code)]
    ...
    /// (`jerry_git::merge::attempt_merge_into_current`), the opposite direction from
    ...
    /// Surface D's `Abort merge` action - `git merge --abort` (`jerry_git::merge::abort_merge`),
    ...
    /// [`crate::code_surface::edit_buffer::EditBuffer`] from [`jerry_git::merge::ConflictedFile::render`]'s
    ...
    /// [`jerry_git::merge::load_conflicted_file`] re-parse of what was just written; (c) if that
    /// re-parse reports zero remaining conflicts, a real [`jerry_git::merge::write_resolved_file`]
    ...
    /// Applies a hand-edit save's fresh, re-parsed [`jerry_git::merge::ConflictedFile`] to the
...
/// (`jerry_git::merge::load_conflicted_file`), *after* the real `std::fs::write` itself already
...
enum MergeEditReparseOutcome {
    /// The written content parsed as a real, valid [`jerry_git::merge::ConflictedFile`] - genuinely
    ...
    /// add` inside `jerry_git::merge::write_resolved_file` itself failed - a real, independent
    /// failure mode from parsing, surfaced as its own distinct, non-blocking warning rather than
    /// discarding a perfectly good `fresh` over it. `jerry_git::merge::complete_merge`'s own
...
/// `abortable_worktree` via [`jerry_git::merge::find_in_progress_merge`] rather than assuming a
...
pub(in crate::merge) fn merge_error_state(
    ...
    let abortable_worktree = jerry_git::merge::find_in_progress_merge(repo_path)
        .ok()
        .flatten();
    merge::MergeFlowState::Error {
        message,
...
/// the same idiom `jerry_git::merge`'s own test module uses), through a real `AdeApp` in a test
...
mod merge_regression_tests {
    ...
    /// Same real-linked-worktree idiom as `jerry_git::merge`'s own test module. The path is
    ...
    /// Real parent-commit count for `rev` - the same real check `jerry_git::merge`'s own test
    ...
    fn merge_head_exists(dir: &std::path::Path) -> bool {
        jerry_git::merge::merge_head_exists(dir).expect("merge_head_exists")
    }
    ...
    fn close_agent_during_in_flight_complete_merge_leaves_repo_usable_for_a_new_merge(
        ...
        assert!(!jerry_git::merge::merge_head_exists(repo.path()).expect("merge_head_exists"));
    ...
    fn resolving_two_files_back_to_back_writes_both_to_disk_without_cancelling_either(
        ...
        app.update(cx, |app, cx| {
            app.resolve_active_hunk(jerry_git::merge::ConflictChoice::Both, cx);
        });
    ...
    fn ending_a_merge_flow_tears_down_hand_edit_state_and_a_fresh_merge_cannot_be_polluted_by_it(
        ...
        let injected = jerry_git::merge::load_conflicted_file(repo.path(), Path::new("shared.txt"))
    ...
    fn saving_malformed_markers_still_records_the_real_write_without_corrupting_files(
        ...
        // `jerry_git::merge::parse_conflict_segments`'s own real parser.
        let malformed =
            "line1\n<<<<<<< HEAD\nBASE CHANGED\nFEATURE CHANGED\n>>>>>>> feature\nline3\n";
        cx.simulate_keystrokes(&secondary("a"));
        app.update_in(cx, |app, window, cx| {
