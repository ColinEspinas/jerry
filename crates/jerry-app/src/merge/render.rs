...
impl AdeApp {
    ...
    /// content) comes from the `jerry_git::merge` call [`Self::start_merge`] made.
    ...
    /// not a whole file - unlike `jerry_git::diff`'s hunks, `ConflictHunk` has no line cap, but a
            ...
            .child(
                div()
                    ...
                    .child(column(
                        ...
                        "take-left",
                        "Take left",
                        jerry_git::merge::ConflictChoice::Left,
                        rem_px,
                        cx,
                    )),
            )
            .child(
                div()
                    ...
                    .child(column(
                        ...
                        "take-right",
                        "Take right",
                        jerry_git::merge::ConflictChoice::Right,
                        rem_px,
                        cx,
                    )),
            )
    ...
    /// The `Take both` action on the active hunk - `jerry_git::merge::ConflictChoice::Both` keeps
            ...
            .child(
                div()
                    ...
                    .on_click(cx.listener(|this, _event: &ClickEvent, _window, cx| {
                        this.resolve_active_hunk(jerry_git::merge::ConflictChoice::Both, cx);
                    })),
            )
    }
    ...
    /// The distinct panel for [`jerry_git::merge::ConflictedPath::Unmergeable`] entries - modify/
    ...
    pub(in crate::merge) fn render_unmergeable_panel(
        &self,
        paths: Vec<(&std::path::Path, jerry_git::merge::UnmergeableReason)>,
    ) -> impl IntoElement {
        div()
            .flex_1()
            .min_h_0()
            ...
            .children(paths.into_iter().map(|(path, reason)| {
                let reason_label = match reason {
                    jerry_git::merge::UnmergeableReason::ModifyDelete => {
                        "modified on one side, deleted on the other"
                    }
                    jerry_git::merge::UnmergeableReason::Binary => "binary content conflict",
                };
                div()
                    .flex()
                    .items_center()
