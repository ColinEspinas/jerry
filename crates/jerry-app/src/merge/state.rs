...
//! only holds already-computed facts (from `jerry_git::merge` calls made in `crate::root`, which
...
use jerry_git::merge::{ConflictSegment, ConflictedFile, ConflictedPath, UnmergeableReason};
...
pub enum MergeFlowState {
    ...
    /// explicit `complete_merge` click (see `jerry_git::merge::complete_merge`'s docs for why
    ...
    /// truth (`jerry_git::merge::classify_conflicted_file`) into either a resolvable text
    ...
    /// An error from `jerry_git::merge` (a refused dirty base worktree, no detectable base
    /// branch, the base branch not checked out anywhere, a git failure, ...) or from
    /// resolving/writing a conflicted file. Repository state is left exactly as `jerry_git::merge`
    /// (or git itself) left it - never silently discarded.
    Error {
        message: String,
        /// The base worktree to offer an `Abort merge` action against, if
        /// `jerry_git::merge::find_in_progress_merge`/`merge_head_exists` found `MERGE_HEAD`
...
/// conflict markers (`jerry_git::merge::load_conflicted_file`). Returns `false` (a no-op) if no
