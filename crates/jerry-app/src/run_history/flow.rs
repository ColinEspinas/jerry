            ...
            let measured = match baseline {
                Some((tree_id, untracked)) => {
                    cx.background_executor()
                        .spawn(async move {
                            jerry_git::review::diff_against_tree(
                                &measure_worktree,
                                &tree_id,
                                untracked,
                                // The label only ever appears in `jerry_git`'s own error text; this
                ...
                .spawn(async move {
                    wanted
                        ...
                        .map(|(path, runs)| {
                            ...
                            let counts = jerry_git::run_drift::commits_since_each(&path, &moments);
