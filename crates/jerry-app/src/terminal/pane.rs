...
//! `claude`) spawned via `jerry-pty`, streamed into a real `alacritty_terminal`-backed
...
impl TerminalSpec {
    ...
    /// binary" choice. A bare name, not an absolute path: `jerry_pty::spawn`'s own
    ...
    /// separator): `jerry-pty`'s `spawn` resolves it via `PATH` through
...
/// already on a real `PATH` and `jerry_pty::login_shell` already covers the GUI-launch case, so
...
mod shell_program_tests {
    ...
    fn a_configured_shell_replaces_the_environment_default() {
        ...
        assert_eq!(
            ...
            PathBuf::from("fish"),
            "a bare name must reach the spawn as-is, for jerry-pty's own PATH resolution"
        );
        assert_eq!(
    ...
    /// `jerry_pty::login_shell` already merges a login `PATH` into the process environment - so
    ...
    fn a_real_login_bash_reaches_usr_bin_from_a_bare_windows_path() {
        let Some(bash) = jerry_pty::resolve_on_path("bash.exe") else {
        ...
        let output = jerry_pty::new_std_command(&spec.program)
            .args(&spec.args)
            .args(["-c", "command -v ls"])
    ...
    fn a_configured_shell_really_spawns_and_a_typo_really_fails() {
        ...
        let mut session = jerry_pty::spawn(
            jerry_pty::SpawnOptions::new(good.program.clone()).cwd(good.cwd.clone()),
        )
        ...
        match jerry_pty::spawn(jerry_pty::SpawnOptions::new(bad.program).cwd(bad.cwd)) {
            Err(err) => assert!(
                matches!(err, jerry_pty::PtyError::Spawn(_)),
...
impl TerminalPane {
    ...
    /// [`Self::resume`] and `jerry_pty::PtySession::pause`'s own docs for the platform scope
    ...
    /// to call even if the process was never actually paused - see `jerry_pty::PtySession::
    /// resume`'s own docs.
...
/// (`Render::render`'s `spawn_error` child), which is plain text `jerry_pty::spawn` returned,
...
mod pty_pane_fixtures {
    ...
    /// `cargo test -p jerry-app --lib terminal::` run before this existed, this module alone held **29
    /// live children at once**. GitHub issue #427 counted 509 across all of `crates/jerry-app`. It does
...
/// (`jerry_pty`'s descendant walk read `/proc`, which does not exist there), and `jerry_pty`'s own
...
///
/// unix-only, like `jerry-pty`'s own teardown suite: it needs real process-group and
...
mod pane_teardown_tests {
    ...
    /// through `jerry_pty`'s own (private) `pid_exists` - so what is checked here is independent
