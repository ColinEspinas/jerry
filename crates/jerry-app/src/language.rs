...
use gpui::Rgba;
use jerry_lsp::lsp_types::request::{
    Completion, GotoDefinition, HoverRequest, Request as LspRequest,
};
use jerry_lsp::{ServerSpawnConfig, WorkspaceConfigFn};
...
/// [`jerry_lsp::ServerSpawnConfig`] directly; [`entry_for_extension`]'s callers turn this plus a
...
pub struct CompanionServer {
    ...
    /// lsp_clients`, and the real `name` its `jerry_lsp::LspClient` reports in every log/status
...
fn vue_typescript_plugin_location() -> Result<String, String> {
    let binary = jerry_pty::resolve_on_path("vue-language-server").ok_or_else(|| {
...
/// `jerry-lsp` generalization docs. Real `pythonPath` detection via `jerry_pty::resolve_on_path`
...
fn pyright_initialization_options() -> Option<serde_json::Value> {
    ...
    let python_path = jerry_pty::resolve_on_path("python3")
        .or_else(|| jerry_pty::resolve_on_path("python"))
...
/// Pyright's real `workspace/configuration` answers - see `jerry_lsp::client`'s generalization
...
pub fn server_spawn_config(
    ...
    Ok(Some(ServerSpawnConfig {
        ...
        // [`jerry_lsp::ServerSpawnConfig::custom_notification_methods`]).
