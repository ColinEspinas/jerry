...
use jerry_git::graph::{DotKind, ElbowKind, Graph, GraphRow, GraphScope, RefKind};
use std::ops::Range;
use std::time::{Instant, SystemTime, UNIX_EPOCH};

/// GitHub issue #221: how many further commits each "load more" adds to the walk cap.
const LOAD_MORE_BATCH: usize = jerry_git::graph::DEFAULT_MAX_COMMITS;
...
impl AdeApp {
    ...
    pub(crate) fn open_git_graph(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // `self.diff_root`, an empty `PathBuf` in this state, which `jerry_git::graph::build_graph`
    ...
    pub(crate) fn close_git_graph_tab(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.graph_state.loaded_cap = jerry_git::graph::DEFAULT_MAX_COMMITS;
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                ...
                .spawn({
                    let root = root.clone();
                    async move {
                        let graph = jerry_git::graph::build_graph(&root, scope, 0);
                        let upstream = jerry_git::graph::ahead_behind_against_upstream(&root);
                        (graph, upstream)
                    }
                })
                .await;
    ...
    pub(crate) fn load_more_graph_rows(&mut self, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move { jerry_git::graph::build_graph(&root, scope, next_cap) })
                .await;
            let _ = this.update(cx, |this, cx| {
    ...
    /// `jerry_git::graph::commit_changed_files` performs real blocking I/O (spawns `git show`), so
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let result = cx
                ...
                .spawn({
                    let root = root.clone();
                    let sha = sha.clone();
                    async move { jerry_git::graph::commit_changed_files(&root, &sha) }
                })
                .await;
            let _ = this.update(cx, |this, cx| {
    ...
    pub(crate) fn request_graph_fetch(&mut self, cx: &mut Context<Self>) {
        self.run_graph_remote_op("Fetch", cx, |root| jerry_git::remote::fetch(&root));
    }
    ...
    /// `jerry_git::remote`'s own module docs on why this doesn't attempt to resolve one itself) -
    ...
    pub(crate) fn request_graph_pull(&mut self, cx: &mut Context<Self>) {
        self.run_graph_remote_op("Pull", cx, |root| jerry_git::remote::pull(&root));
    }
    ...
    /// (`jerry_git::remote::PushForce::Force`'s own docs), so this applies the same two-click
    ...
    /// second click on that *same* force value is what actually runs [`jerry_git::remote::push`].
    pub(crate) fn request_graph_push(
        &mut self,
        force: jerry_git::remote::PushForce,
        cx: &mut Context<Self>,
    ) {
        ...
        self.run_graph_remote_op(action, cx, move |root| {
            jerry_git::remote::push(&root, force)
        });
    }
    ...
    /// (`jerry_git::rewrite::cherry_pick`). A real conflict surfaces through
    ...
    pub(crate) fn request_graph_cherry_pick(&mut self, sha: String, cx: &mut Context<Self>) {
        ...
        self.run_graph_remote_op("Cherry-pick", cx, move |root| {
            jerry_git::rewrite::cherry_pick(&root, &sha)
        });
    }

    /// The row menu's "Revert" action - GitHub issue #1's own "revert commit". Creates a new
    /// commit undoing `sha`'s changes (`jerry_git::rewrite::revert`); see
    ...
    pub(crate) fn request_graph_revert(&mut self, sha: String, cx: &mut Context<Self>) {
        ...
        self.run_graph_remote_op("Revert", cx, move |root| {
            jerry_git::rewrite::revert(&root, &sha)
        });
    }

    /// The row menu's "Check out" action (GitHub issue #241). Moves `HEAD` (detached) onto `sha`
    /// in the focused worktree (`jerry_git::checkout::checkout`) - never the repository's main
    ...
    /// or second-guess a dirty worktree itself; see `jerry_git::checkout::checkout`'s own docs.
    pub(crate) fn request_graph_checkout(&mut self, sha: String, cx: &mut Context<Self>) {
        ...
        self.run_graph_remote_op("Check out", cx, move |root| {
            jerry_git::checkout::checkout(&root, &sha)
        });
    }
    ...
    /// Enter on the branch-name prompt - runs whichever real `jerry_git::checkout` mutation this
    ...
    /// The real `jerry_git::checkout::create_branch_at` call behind
    ...
    pub(crate) fn request_graph_create_branch(
        ...
        self.run_graph_remote_op("Create branch", cx, move |root| {
            jerry_git::checkout::create_branch_at(&root, &name, &sha)
        });
    }

    /// The real `jerry_git::checkout::rename_branch` call behind
    ...
    pub(crate) fn request_graph_rename_branch(
        ...
        self.run_graph_remote_op("Rename branch", cx, move |root| {
            jerry_git::checkout::rename_branch(&root, &old_name, &new_name)
        });
    }

    /// The branch menu's "Checkout Branch" action (GitHub issue #241) - `git switch -- <branch>`
    /// in the focused worktree (`jerry_git::checkout::checkout_branch`), landing on the *branch*
    ...
    pub(crate) fn request_graph_branch_checkout(&mut self, branch: String, cx: &mut Context<Self>) {
        self.graph_state.delete_branch_confirm_armed = None;
        self.run_graph_remote_op("Check out", cx, move |root| {
            jerry_git::checkout::checkout_branch(&root, &branch)
        });
    }
    ...
    /// of that specific branch (`jerry_git::remote::push_branch`), regardless of what is checked
    ...
    pub(crate) fn request_graph_push_branch(&mut self, branch: String, cx: &mut Context<Self>) {
        ...
        self.run_graph_remote_op("Push branch", cx, move |root| {
            jerry_git::remote::push_branch(&root, &branch, jerry_git::remote::PushForce::None)
        });
    }

    /// The branch menu's "Delete Branch…" action (GitHub issue #241) - real `git branch -d`
    /// (`jerry_git::checkout::delete_branch`), behind the same two-click confirmation
    ...
    pub(crate) fn request_graph_delete_branch(&mut self, branch: String, cx: &mut Context<Self>) {
        ...
        self.run_graph_remote_op("Delete branch", cx, move |root| {
            jerry_git::checkout::delete_branch(&root, &branch)
        });
    }

    /// The row menu's "Soft"/"Mixed"/"Hard" reset actions (GitHub issue #241) - real `git reset`
    /// against the focused worktree's current branch (`jerry_git::checkout::reset`).
    pub(crate) fn request_graph_reset(
        &mut self,
        mode: jerry_git::checkout::ResetMode,
        sha: String,
        cx: &mut Context<Self>,
    ) {
        use jerry_git::checkout::ResetMode;
        ...
        self.run_graph_remote_op(action, cx, move |root| {
            jerry_git::checkout::reset(&root, mode, &sha)
        });
    }
    ...
    /// executor (every `jerry_git::remote` function performs real blocking I/O), then surfaces a
    ...
    fn run_graph_remote_op(
        ...
        action: &'static str,
        cx: &mut Context<Self>,
        op: impl FnOnce(std::path::PathBuf) -> Result<(), jerry_git::Error> + Send + 'static,
    ) {
        if self.graph_state.remote_op_in_flight {
            self.report_graph_op_in_flight(cx);
            return;
...
impl AdeApp {
    ...
    /// segment, and the Fetch/Pull/Push button group. Fetch and Pull run real `jerry_git::remote`
    ...
    /// Force). All three entries are real `jerry_git::remote::push` calls
    ...
    fn render_graph_push_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // fabricated "↑0" - matching `jerry_git::graph::ahead_behind_against_upstream`'s own "no
            ...
            .child(
                ...
                .child(
                    ...
                    .on_click(cx.listener(
                        |this, _event: &ClickEvent, _window, cx| {
                            cx.stop_propagation();
                            this.request_graph_push(jerry_git::remote::PushForce::None, cx);
                        },
                    )),
                )
                .child(
                    render_dropdown_menu_row(
                        ...
                        if self.graph_state.push_force_confirm_armed
                            == Some(jerry_git::remote::PushForce::WithLease)
                        {
                    ...
                    .on_click(cx.listener(
                        |this, _event: &ClickEvent, _window, cx| {
                            cx.stop_propagation();
                            this.request_graph_push(jerry_git::remote::PushForce::WithLease, cx);
                        },
                    )),
                )
                .child(
                    render_dropdown_menu_row(
                        ...
                        "Force",
                        if self.graph_state.push_force_confirm_armed
                            == Some(jerry_git::remote::PushForce::Force)
                        {
                            "click again to really push".to_string()
                        } else {
                    ...
                    .on_click(cx.listener(
                        |this, _event: &ClickEvent, _window, cx| {
                            cx.stop_propagation();
                            this.request_graph_push(jerry_git::remote::PushForce::Force, cx);
                        },
                    )),
                ),
            )
    ...
    /// entry is real (`jerry_git::checkout`/`jerry_git::rewrite`/the `jerry_git::rebase`-backed rebase
            ...
            .child(
                    ...
                    .on_click(cx.listener({
                        ...
                        move |this, _event: &ClickEvent, _window, cx| {
                            this.request_graph_reset(
                                jerry_git::checkout::ResetMode::Soft,
                                sha.clone(),
                                cx,
                            );
                        }
                ...
                .child(
                    ...
                    .on_click(cx.listener({
                        ...
                        move |this, _event: &ClickEvent, _window, cx| {
                            this.request_graph_reset(
                                jerry_git::checkout::ResetMode::Mixed,
                                sha.clone(),
                                cx,
                            );
                        }
                ...
                .child(
                    ...
                    .on_click(cx.listener({
                        ...
                        move |this, _event: &ClickEvent, _window, cx| {
                            this.request_graph_reset(
                                jerry_git::checkout::ResetMode::Hard,
                                sha.clone(),
                                cx,
                            );
                        }
...
fn render_graph_file_row(file: jerry_git::graph::CommitFileChange) -> impl IntoElement {
...
fn elbow_paint_order(elbows: &[jerry_git::graph::Elbow]) -> Vec<(usize, jerry_git::graph::Elbow)> {
    let mut ordered: Vec<(usize, jerry_git::graph::Elbow)> =
...
fn lane_segment_span(
    segment: &jerry_git::graph::LaneSegment,
    row_h: Pixels,
    grid: SnapGrid,
) -> (Pixels, Pixels) {
...
fn lane_for_ref(row: &GraphRow, _chip: &jerry_git::graph::RefChip) -> usize {
    row.lane
}
...
mod elbow_geometry_tests {
    ...
    /// Every shape `elbow_geometry` can produce: both kinds, both directions. `jerry_git::graph`'s
    ...
    fn what_the_row_clip_removes_is_exactly_what_the_neighbouring_row_paints() {
        ...
        // stays exactly at 0).
        let through = jerry_git::graph::LaneSegment {
            lane: 1,
            starts_here: false,
            ends_here: false,
            dashed: false,
    ...
    fn a_lane_that_starts_or_ends_here_gets_exactly_one_half_height_stub() {
        ...
        let mut segment = jerry_git::graph::LaneSegment {
            lane: 1,
            starts_here: true,
            ends_here: false,
            dashed: false,
    ...
    fn every_elbow_in_a_crowded_row_keeps_its_own_lanes_arc_visible() {
        ...
        for (kind, far_lanes, dot_lane) in [
            ...
            let elbows: Vec<jerry_git::graph::Elbow> = far_lanes
                .iter()
                .map(|&far| match kind {
                    ElbowKind::Converging => jerry_git::graph::Elbow {
                        from_lane: far,
                        to_lane: dot_lane,
                        kind,
                    },
                    ElbowKind::Diverging => jerry_git::graph::Elbow {
                        from_lane: dot_lane,
                        to_lane: far,
                        kind,
                    },
...
mod elbow_fractional_scale_tests {
    ...
    fn consecutive_rows_of_one_lane_leave_no_blank_device_row_at_any_scale() {
        ...
        let starts = jerry_git::graph::LaneSegment {
            starts_here: true,
            ..through
        };
        for scale in swept_scales() {
...
/// ([`jerry_git::graph::ElbowKind`]) - a real regression test for the git graph tab's "start of
...
/// direct call into the pure layout function - `jerry_git::graph`'s own unit tests already cover
...
mod graph_elbow_render_tests {
    ...
    fn expected_stroke_x(
        ...
        let leaves_dot = match elbow.kind {
            jerry_git::graph::ElbowKind::Diverging => is_entry,
            jerry_git::graph::ElbowKind::Converging => !is_entry,
        };
    ...
    fn a_converging_elbow_paints_in_the_rows_top_half_on_the_ending_lanes_side(
        ...
        assert!(
            elbow_kinds
                .iter()
                .all(|elbow| elbow.kind == jerry_git::graph::ElbowKind::Converging),
    ...
    fn a_diverging_elbow_paints_in_the_rows_bottom_half(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            elbow_kinds,
            vec![jerry_git::graph::Elbow {
                from_lane: 0,
                to_lane: 1,
                kind: jerry_git::graph::ElbowKind::Diverging,
            }],
...
/// `request_graph_push` really shelling out to `jerry_git::remote`, not just the not-yet-wired
/// stub these replaced.
#[cfg(test)]
mod graph_remote_action_tests {
    ...
    async fn hard_reset_requires_a_real_second_click_before_it_runs(cx: &mut TestAppContext) {
        ...
        app.update_in(cx, |app, _window, cx| {
            app.request_graph_reset(jerry_git::checkout::ResetMode::Hard, base_sha.clone(), cx);
        });
        cx.run_until_parked();
...
mod graph_virtualization_tests {
    ...
    /// that in every direction, and still well under `jerry_git::graph::DEFAULT_MAX_COMMITS`, so
    ...
    fn the_load_more_row_is_the_last_item_of_the_list_at_the_shared_row_height(
        ...
        let truncated = jerry_git::graph::build_graph(repo.path(), GraphScope::All, 3)
    ...
    /// GitHub issue #221 seeds: more commits than `jerry_git::graph::DEFAULT_MAX_COMMITS`, so the
    ...
    fn scrolling_to_the_bottom_of_a_capped_walk_really_loads_more_commits(cx: &mut TestAppContext) {
        ...
        app.read_with(cx, |app, _| {
            ...
            assert_eq!(
                graph.rows.len(),
                jerry_git::graph::DEFAULT_MAX_COMMITS,
        ...
        app.read_with(cx, |app, _| {
            ...
            assert_eq!(
                ...
                jerry_git::graph::DEFAULT_MAX_COMMITS + LOAD_MORE_BATCH,
...
mod graph_branch_menu_tests {
    ...
    fn merge_row_is_disabled_with_the_designs_own_reason_while_the_worktree_is_dirty(
        ...
        // `jerry_git::stage::dirty_paths` read is what fills `dirty_files` - never a hand-set field.
        ...
        assert!(
            !jerry_git::merge::merge_head_exists(repo.path()).expect("merge_head_exists"),
...
mod graph_branch_action_tests {
    ...
    async fn merge_into_current_branch_lands_in_the_existing_merge_flow(cx: &mut TestAppContext) {
        ...
        assert!(
            jerry_git::merge::merge_head_exists(local.path()).expect("merge_head_exists"),
        ...
        app.read_with(cx, |app, _| {
            match &app.merge_flow.as_ref().expect("merge flow").state {
                merge::MergeFlowState::Conflicted { files, .. } => {
                    ...
                    let jerry_git::merge::ConflictedPath::Text(file) = &files[0] else {
                    ...
                    let hunk = file
                        ...
                        .find_map(|segment| match segment {
                            jerry_git::merge::ConflictSegment::Conflict(hunk) => Some(hunk),
                            _ => None,
                        })
