...
use jerry_git::graph::{Graph, GraphScope};
...
pub(crate) struct GraphBranchMergeFacts {
    ...
    /// state `jerry_git::merge::attempt_merge_into_current` refuses with `Error::MergeTargetDetached`.
    pub current_branch: Option<String>,
    /// The branch the merge would take commits *from*: the one whose row the menu was opened on.
    pub source_branch: String,
    /// How many real uncommitted paths the focused worktree has (`jerry_git::stage::dirty_paths`,
...
pub(crate) struct GraphTabState {
    ...
    /// `jerry_git::graph::commit_changed_files` performs real blocking I/O (spawns `git show`) and
    ...
    pub commit_files_cache: Option<(
        String,
        Result<Vec<jerry_git::graph::CommitFileChange>, String>,
    )>,
    ...
    /// (`jerry_git::graph::ahead_behind_against_upstream`), refreshed alongside every
    ...
    pub upstream_counts: Option<jerry_git::diff::AheadBehind>,
    ...
    /// `true` while a real `jerry_git::remote::{fetch,pull,push}` call is running on the
    ...
    /// see `jerry_git::remote::PushForce::Force`'s own docs) - `None` for the ordinary, single-
    ...
    /// [`jerry_git::remote::push`]. Clicking a different row (including the plain "Push" row)
    ...
    /// nothing else clicked in between - actually runs [`jerry_git::checkout::reset`]. Keyed by
    ...
    /// [`jerry_git::checkout::delete_branch`]. Keyed by branch name rather than a bare `bool` for
    ...
    /// currently loaded [`Graph`] was really walked with - `jerry_git::graph::DEFAULT_MAX_COMMITS`
...
impl GraphTabState {
    pub(crate) fn new(cx: &mut Context<AdeApp>) -> Self {
        Self {
            ...
            loaded_cap: jerry_git::graph::DEFAULT_MAX_COMMITS,
