...
use jerry_git::rebase::RebaseOutcome;
...
impl AdeApp {
    ...
    fn render_rebase_banner_actions(
        ...
        // load, or a real `jerry_git::rebase` call) was already in flight - a double-click on
    ...
    fn a_reword_message_supplied_before_start_runs_straight_through_with_no_stop(
        ...
        app.read_with(cx, |app, _| {
            ...
            assert_eq!(
                ...
                jerry_git::rebase::RebaseAction::Reword(Some(
                    "third supplied up front".to_string()
                )),
                "the real plan entry handed to jerry_git must carry the supplied message, so the \
