...
//! (entering/leaving, editing the in-memory plan, driving `jerry_git::rebase` for real); rendering
...
/// `jerry_git::rebase::RebaseAction` that keeps `Reword`'s optional message in
...
/// `jerry_git::rebase::RebasePlanEntry` (which only ever exists transiently, built fresh by
...
pub(crate) struct RebasePlanRow {
    ...
    /// `jerry_git::rebase::RebasePlanEntry::commit`'s own contract.
    pub commit: String,
    ...
    /// The real count of files this commit changed (`jerry_git::graph::commit_changed_files`),
...
impl RebasePlanRow {
    ...
    /// `jerry_git::rebase`. `Reword` maps to `Some(message)` only when
    /// [`Self::has_supplied_reword_message`] is true - a message-less reword genuinely means
    /// "stop here" ([`jerry_git::rebase::RebaseAction::Reword`]'s own docs), never fabricated.
...
pub(crate) struct RebaseModeState {
    ...
    pub branch: String,
    /// The real commit-ish `jerry_git::rebase::start_interactive_rebase` bases the plan onto - the
    ...
    /// Real ids `jerry_git::graph::commits_already_on_upstream` reported for this plan's commits -
...
impl AdeApp {
    ...
    /// (`jerry_git::rebase::commits_to_rebase`'s own contract: every commit `HEAD` has that isn't
    ...
    pub(in crate::graph_view) fn enter_rebase_mode_onto_branch(
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let resolved = cx
                .background_executor()
                .spawn(async move { jerry_git::graph::resolve_commit(&root, &branch_for_bg) })
                .await;
    ...
    pub(in crate::graph_view) fn enter_rebase_mode_inner(
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let result = cx
                .background_executor()
                .spawn(async move {
                    let commits = jerry_git::rebase::commits_to_rebase(&root_for_bg, &onto_for_bg)?;
                    let already_on_upstream =
                        jerry_git::graph::commits_already_on_upstream(&root_for_bg, &commits)
                            .ok()
                            .flatten()
                            .unwrap_or_default();
                    let files: Vec<(String, Option<usize>)> = commits
                        .iter()
                        .map(|id| {
                            let count = jerry_git::graph::commit_changed_files(&root_for_bg, id)
                                .ok()
                                .map(|files| files.len());
                            (id.clone(), count)
                        })
                        .collect();
                    Ok::<_, jerry_git::Error>((commits, already_on_upstream, files))
                })
                .await;
    ...
    /// every commit `jerry_git::rebase::commits_to_rebase` can return for a row opened from this
        ...
        let amend = if let RebasePhase::Stopped {
            outcome:
                RebaseOutcome::StoppedForEdit {
                    commit,
                    reason: Some(jerry_git::rebase::StopReason::RewordNeedsMessage),
                },
        } = &rebase_state.phase
        {
            rebase_state
