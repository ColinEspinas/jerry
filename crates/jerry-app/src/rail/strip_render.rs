...
impl AdeApp {
    ...
    fn compute_worktree_problems(&self, root: &Path) -> Vec<Problem> {
        ...
        // A diagnostic's uri is whatever real path the *server* opened, and `jerry_lsp`'s own
        ...
        // `jerry_lsp` canonicalises with `dunce` too - std's Windows verbatim `\\?\` spelling
...
mod problems_view_tests {
    ...
    fn real_file(root: &Path, relative: &str, contents: &str) -> (PathBuf, String) {
        ...
        let uri = jerry_lsp::LspClient::uri_for_path(&absolute)
