//! Maps `jerry-git`'s [`jerry_git::WorktreeStatus`] list (from `jerry_git::list_worktrees_porcelain`,
...
pub struct WorktreeItem {
    ...
    /// main *worktree* to distinguish (mirrors `jerry_git::list_worktrees`'s own rule).
    pub is_main: bool,
...
/// Converts `jerry-git`'s real `git worktree list --porcelain` results into display rows. See
/// this module's own docs for why every consumer of the worktree list now goes through this one
/// function.
pub fn build_worktree_items(results: Vec<jerry_git::WorktreeStatus>) -> Vec<WorktreeItem> {
    results
        .into_iter()
        .map(|status| {
