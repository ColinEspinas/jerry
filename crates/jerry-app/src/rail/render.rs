...
impl AdeApp {
    ...
    /// Builds the worktree list: every worktree `jerry_git::list_worktrees` reported, including
                    ...
                    .spawn(async move {
                        ...
                        let review_measurements: Vec<(
                            ...
                            .filter_map(|(id, worktree, tree_id, untracked)| {
                                let paths = jerry_git::review::changed_paths_against_tree(
    ...
    /// `jerry_git::is_dirty` follows git's ignored-file semantics, so a "clean" worktree can
    ...
    /// Removes `candidates` via `jerry_git::remove_worktree`. Only called once
                ...
                .spawn({
                    ...
                    async move {
                        ...
                        for path in candidates {
                            match jerry_git::remove_worktree(&repo_path, &path, false) {
                                Ok(()) => removed += 1,
    ...
    pub(in crate::rail) fn render_rail_footer(&self, cx: &mut Context<Self>) -> impl IntoElement {
        // Includes error'd entries - the count should match what `jerry_git::list_worktrees`
...
/// `jerry_git::remove_worktree` has run - must leave exactly one batch in flight, never two
...
mod prune_regression_tests {
    ...
    fn seed_one_prunable_worktree(app: &mut AdeApp, path: PathBuf, branch: &str) {
        ...
        app.worktree_notes.insert(
            ...
            WorktreeNote {
                is_main: false,
                clean: Some(true),
                merge: Some(jerry_git::diff::WorktreeMergeStatus {
    ...
    fn a_second_confirm_while_first_batch_is_in_flight_does_not_prune_a_worktree_seeded_after_it(
        ...
        // `jerry_git::remove_worktree` has not executed yet.
        app.update(cx, |app, cx| app.request_prune(cx));
        assert!(
...
mod worktree_tab_attribution_tests {
    ...
    /// A real repository with a real commit - `jerry_git::list_worktrees_porcelain` reports nothing
