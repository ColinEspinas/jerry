...
//! `crate::root` gathers the real signals (`TerminalPane`, `jerry_git::list_worktrees`,
//! `jerry_git::diff::diff_against_base`) into the plain types this module operates on, and renders
...
use jerry_git::diff::{AheadBehind, DiffLineKind, WorktreeDiff, WorktreeMergeStatus};

/// One agent, reduced to exactly what the rail row needs to render - built in `crate::root`
/// from a `crate::work_surface::agents::Agent` plus a `jerry_git::diff::diff_against_base` result for its
...
pub struct AgentRow {
    ...
    /// `+added -deleted` line counts from `jerry_git::diff::diff_against_base`, summed across
...
/// the full line-level diff is already loaded by `jerry_git::diff::diff_against_base`, so this
...
/// `jerry_git::is_dirty` and `jerry_git::diff::merge_status_against_base`. See [`Self::label`]
...
pub struct WorktreeNote {
    ...
    pub is_main: bool,
    /// `None` if `jerry_git::is_dirty` itself failed (a blank note rather than a guess).
    ...
    /// From `jerry_git::Worktree::is_locked` - a locked worktree is never offered as prunable
...
impl WorktreeNote {
    ...
    /// `jerry_git::remove_worktree`'s own dirty-tree refusal up front), and merged into its
    /// detected base.
...
/// `jerry_git::WorktreeResult` (via `crate::rail::worktrees::WorktreeItem`) plus its separately
...
pub struct RepoWorktrees {
    ...
    /// real `jerry_git::list_worktrees_porcelain` fetch for it has completed at least once
...
pub struct StatusSnapshot {
    ...
    /// Real `jerry_git::diff::ahead_behind_against_base` result per worktree/agent cwd - the
...
pub fn compute_status_snapshot(
    ...
    for path in unique_diff_paths {
        let summary = match jerry_git::diff::diff_against_base(&path) {
        ...
        if let Ok(Some(counts)) = jerry_git::diff::ahead_behind_against_base(&path) {
    ...
    for query in worktrees {
        let clean = jerry_git::is_dirty(&query.path).ok().map(|dirty| !dirty);
        let merge = if query.is_main {
            None
        } else {
            jerry_git::diff::merge_status_against_base(&query.path)
                .ok()
                .flatten()
        };
...
mod tests {
    ...
    /// clean and unlocked. A dirty one would be refused by `jerry_git::remove_worktree` anyway; a
    ...
    fn sum_diff_stat_counts_added_and_removed_lines_across_files_and_hunks() {
        use jerry_git::diff::{DiffFile, DiffHunk, DiffLine, FileChangeStatus};
