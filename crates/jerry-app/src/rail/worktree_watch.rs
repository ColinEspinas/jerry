...
pub fn spawn_worktree_watcher(repo_path: &Path, dirty: DirtyFlag) -> Option<RecommendedWatcher> {
    let common_dir = jerry_git::git_common_dir(repo_path).ok()?;
...
///   them at all);
/// - by *name*: the [`jerry_git::diff::SHADOW_INDEX_PREFIX`] tempfiles every diff computation
        ...
        && event.paths.iter().all(|path| {
            let neutral_name =
                path.file_name()
                    ...
                    .is_some_and(|name| {
                        name.starts_with(jerry_git::diff::SHADOW_INDEX_PREFIX)
                            || name == "index"
                            || name == "index.lock"
                            || name == "objects"
                    });
...
mod tests {
    ...
    fn assert_diff_leaves_watcher_clean(repo_path: &Path, diff_target: &Path) {
        ...
        let common_dir = jerry_git::git_common_dir(repo_path).expect("git_common_dir");
        let worktrees_dir = common_dir.join("worktrees");
        let filter_root = worktrees_dir.clone();
        ...
        jerry_git::diff::diff_against_head(diff_target).expect("diff_against_head");
