...
//! `rust-analyzer` via `jerry_lsp::LspClient::request`, see `crate::root::AdeApp::request_hover`)
...
/// exact captured sample `jerry_lsp::client`'s real TypeScript hover test produced), not
...
/// `Vec<LocationLink>`) - `jerry_lsp::client`'s end-to-end definition test observes rust-analyzer
...
mod tests {
    ...
    /// `jerry_lsp::client::tests::typescript_language_server_returns_a_real_hover_for_a_documented_function`,
