...
/// `jerry_lsp::LspClient::pull_diagnostics` call succeeds but reports an empty result - or
...
/// `jerry_lsp::LspClient`, so `code_surface`'s hover/go-to-definition/render paths never need to
/// know or care which it is.
pub(crate) enum LspConnection {
    Single(std::sync::Arc<jerry_lsp::LspClient>),
    WithCompanion {
        primary: std::sync::Arc<jerry_lsp::LspClient>,
        companion: std::sync::Arc<jerry_lsp::LspClient>,
...
impl LspConnection {
    fn primary(&self) -> &std::sync::Arc<jerry_lsp::LspClient> {
        match self {
    ...
    fn companion(&self) -> Option<&std::sync::Arc<jerry_lsp::LspClient>> {
        match self {
            Self::Single(_) => None,
    ...
    pub(crate) fn request<R: jerry_lsp::lsp_types::request::Request>(
        &self,
        params: R::Params,
        timeout: Duration,
    ) -> Result<R::Result, jerry_lsp::LspError> {
        ...
        let replayable = serde_json::to_value(&params).map_err(jerry_lsp::LspError::Serialize)?;
    ...
    /// `jerry_lsp::LspClient::diagnostics_for_uri`'s own real distinction between "haven't heard
    /// back" and "analyzed, found nothing".
    pub(crate) fn diagnostics_for_uri(
        &self,
        uri: &jerry_lsp::lsp_types::Uri,
    ) -> Option<Vec<jerry_lsp::lsp_types::Diagnostic>> {
    ...
    /// `jerry_lsp::LspClient::diagnostics_for`'s own docs).
    pub(in crate::lsp) fn diagnostics_for(
        &self,
        path: &Path,
    ) -> Option<Vec<jerry_lsp::lsp_types::Diagnostic>> {
        let uri = jerry_lsp::LspClient::uri_for_path(path).ok()?;
    ...
    /// message naming **which** one died, by its own `jerry_lsp::LspClient::name()` (which for a
    ...
    pub(in crate::lsp) fn companion_diagnostics_pull_target(
        &self,
    ) -> Option<std::sync::Arc<jerry_lsp::LspClient>> {
        self.companion()
...
struct LspSyncRequest {
    ...
    content: String,
    version: i32,
    /// Whether [`AdeApp::lsp_connection_for_path`]'s client's [`jerry_lsp::LspClient::diagnostics_for_uri`]
...
struct CompletionRequestPlan {
    client: std::sync::Arc<LspConnection>,
    generation: u64,
    params: jerry_lsp::lsp_types::CompletionParams,
...
impl AdeApp {
    ...
    pub(crate) fn take_lsp_clients_for_root(
        &mut self,
        root: &Path,
    ) -> Vec<std::sync::Arc<jerry_lsp::LspClient>> {
    ...
    pub(in crate::lsp) fn reap_dead_lsp_clients(&mut self, cx: &mut Context<Self>) -> bool {
        ...
        for (key, reason) in dead {
            ...
            // `jerry_lsp::LspClient`'s own `Drop` on the GPUI foreground thread - a `/proc`
    ...
    /// Lazily spawns (or reuses) an `jerry_lsp::LspClient` for `repo_root` running the server for
    ...
    pub(crate) fn ensure_lsp_client(
        ...
        let apply_user_options =
            std::sync::Arc::new(move |config: &mut jerry_lsp::ServerSpawnConfig| {
    ...
    fn spawn_lsp_client(
        ...
        key: LspClientKey,
        repo_root: PathBuf,
        build_config: impl FnOnce() -> Result<jerry_lsp::ServerSpawnConfig, String> + Send + 'static,
        cx: &mut Context<Self>,
    ) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                ...
                .spawn(async move {
                    ...
                    jerry_lsp::LspClient::spawn(&repo_root, config).map_err(|err| err.to_string())
                })
                .await;
    ...
    /// `jerry_lsp::LspClient::did_open`'s own docs on why this varies per extension, not just per
            ...
            let uri = cx
                ...
                .spawn({
                    ...
                    async move {
                        ...
                        jerry_lsp::LspClient::uri_for_path(&path).ok()
                    }
                })
                .await;
            if let Some(uri) = uri {
    ...
    /// `jerry_lsp`'s own reader thread, outside the GPUI runtime, with no way to directly notify
            ...
            let update_result = this.update(cx, |this, cx| {
                ...
                for (key, state) in this.lsp_clients.iter() {
                    if let LspClientState::Ready(client) = state {
                        ...
                        // sit at `jerry-lsp`'s own queue cap warning forever. Only a client whose
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let body = match (command_and_args, companion) {
                ...
                (Some((command, args)), Some(companion)) => {
                    let request = jerry_lsp::lsp_types::ExecuteCommandParams {
                    ...
                    let result = cx
                        ...
                        .spawn(async move {
                            companion
                                .request::<jerry_lsp::lsp_types::request::ExecuteCommand>(
                                    request,
                                    LSP_QUERY_TIMEOUT,
                                )
    ...
    pub(crate) fn schedule_lsp_sync(
        ...
        let task = cx.spawn({
            ...
            async move |this, cx| {
                    ...
                    if let Some(request) = completion {
                        ...
                        let completion_task = cx.spawn(async move |cx| {
                            ...
                            let result = cx
                                .background_executor()
                                .spawn(async move {
                                    client.request::<jerry_lsp::lsp_types::request::Completion>(
                                        params,
                                        LSP_QUERY_TIMEOUT,
                                    )
                                })
                ...
                // `textDocument/diagnostic` request. `jerry_lsp::LspClient::
                ...
                if client.supports_diagnostic_pull() {
                    ...
                    for attempt in 0..=max_retries {
                        ...
                        // (Revision R8.5b audit finding 5 - see `jerry_lsp::LspClient::
    ...
    fn prepare_lsp_sync(
        ...
        let position = jerry_lsp::lsp_types::Position {
            line: line as u32,
            character,
        };
        ...
        let completion_context = if force_completion {
            Some(jerry_lsp::lsp_types::CompletionContext {
                trigger_kind: jerry_lsp::lsp_types::CompletionTriggerKind::INVOKED,
        ...
        let completion = match (completion_context, cached_uri) {
            (Some(context), Some(uri)) => {
                ...
                let params = jerry_lsp::lsp_types::CompletionParams {
                    text_document_position: jerry_lsp::lsp_types::TextDocumentPositionParams {
                        text_document: jerry_lsp::lsp_types::TextDocumentIdentifier { uri },
                        position,
                    },
                    work_done_progress_params:
                        jerry_lsp::lsp_types::WorkDoneProgressParams::default(),
                    partial_result_params: jerry_lsp::lsp_types::PartialResultParams::default(),
    ...
    fn apply_completion_result(
        ...
        relative_path: &Path,
        generation: u64,
        result: Result<Option<jerry_lsp::lsp_types::CompletionResponse>, jerry_lsp::LspError>,
        cx: &mut Context<Self>,
    ) {
        ...
        let new_state = match result {
            Ok(Some(response)) => {
                let items = match response {
                    jerry_lsp::lsp_types::CompletionResponse::Array(items) => items,
                    jerry_lsp::lsp_types::CompletionResponse::List(list) => list.items,
                };
    ...
    pub(crate) fn maybe_resolve_selected_completion_item(&mut self, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    connection.request::<jerry_lsp::lsp_types::request::ResolveCompletionItem>(
                        item_to_resolve,
                        LSP_QUERY_TIMEOUT,
                    )
                })
    ...
    pub(crate) fn apply_resolved_completion_item(
        ...
        generation: u64,
        item_index: usize,
        result: Result<jerry_lsp::lsp_types::CompletionItem, jerry_lsp::LspError>,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn described_completion_item<'a>(
        &'a self,
        items: &'a [jerry_lsp::lsp_types::CompletionItem],
        item_index: usize,
    ) -> Option<&'a jerry_lsp::lsp_types::CompletionItem> {
...
pub(crate) enum LanguageServerRunState {
    Ready,
    /// The real `jerry_lsp::LspError`/connection-lost text, shown as-is - the same string
    /// [`LspClientState::Failed`] carries and the File view's own status chip renders.
    Failed(String),
}

/// The state of one repository root's `jerry_lsp::LspClient` across its asynchronous
/// spawn-then-initialize lifecycle (`jerry_lsp::LspClient::spawn` doesn't return until the
...
pub(crate) enum LspClientState {
    ...
    Ready(std::sync::Arc<jerry_lsp::LspClient>),
    /// A spawn/handshake failure (e.g. `rust-analyzer` not on `PATH`) - carries the
    /// `jerry_lsp::LspError`'s own `Display` text, shown as-is rather than a generic message
    /// that would hide *why*.
    Failed(String),
}
...
/// a real `jerry_lsp::LspClient`, which can only be constructed by genuinely spawning a server.
...
pub(crate) enum LspFileStatus {
    ...
    /// `jerry_lsp::LspClient::has_diagnostics_result`'s own docs for the signal this reads).
    Indexing,
    Analyzed {
        errors: usize,
        warnings: usize,
...
/// Takes an already-computed [`jerry_lsp::lsp_types::Uri`] (see [`AdeApp::render_file_view`]'s
...
pub(crate) fn lsp_file_status(
    ...
    uri: Option<&jerry_lsp::lsp_types::Uri>,
) -> LspFileStatus {
    match primary_state {
...
/// rather than real `Arc<jerry_lsp::LspClient>`s - the full end-to-end process-lifecycle proof
  ...
  if (msg.method === 'textDocument/diagnostic') {
    if (MODE === 'pull_flaky') {
      ...
      if (diagnosticPullCalls === 1) {
        // A real, non-ServerCancelled JSON-RPC error - `jerry_lsp::LspClient::pull_diagnostics`
...
"#;
    ...
    /// `initialize`/`initialized` handshake through `jerry_lsp::LspClient::spawn` - the exact same
    /// spawn path every real server in this app goes through, with no test-only shortcut.
    pub(crate) fn spawn_fake_server(
        root: &Path,
        name: &'static str,
        mode: &str,
    ) -> std::sync::Arc<jerry_lsp::LspClient> {
        let config = jerry_lsp::ServerSpawnConfig {
            name,
            binary: "node",
            args: vec![
                "-e".to_string(),
            ...
            workspace_configuration: jerry_lsp::default_workspace_configuration,
        ...
        std::sync::Arc::new(
            jerry_lsp::LspClient::spawn(root, config)
    ...
    fn uri(raw: &str) -> jerry_lsp::lsp_types::Uri {
    ...
    pub(crate) fn publish_and_wait(client: &jerry_lsp::LspClient, target: &str, message: &str) {
        client
            .notify_raw(
                "test/publish",
    ...
    fn wait_for_relay_observation(client: &jerry_lsp::LspClient) -> String {
    ...
    fn hover_falls_back_to_the_companion_when_the_primary_has_nothing() {
        ...
        let params = jerry_lsp::lsp_types::HoverParams {
            text_document_position_params: jerry_lsp::lsp_types::TextDocumentPositionParams {
                text_document: jerry_lsp::lsp_types::TextDocumentIdentifier {
                    uri: uri("file:///hover.vue"),
                },
                position: jerry_lsp::lsp_types::Position {
                    line: 1,
                    character: 7,
                },
            },
        ...
        assert!(
            single
                .request::<jerry_lsp::lsp_types::request::HoverRequest>(
                    params.clone(),
        ...
        let hover = paired
            .request::<jerry_lsp::lsp_types::request::HoverRequest>(params, Duration::from_secs(10))
            .expect("a real response")
            .expect("the companion's real hover should surface through the facade");
        let jerry_lsp::lsp_types::HoverContents::Markup(markup) = hover.contents else {
            panic!("the fake server sends real markup content");
        };
        assert_eq!(markup.value, "real hover from hover");
    }

    fn goto_params(target: &str) -> jerry_lsp::lsp_types::GotoDefinitionParams {
        jerry_lsp::lsp_types::GotoDefinitionParams {
            text_document_position_params: jerry_lsp::lsp_types::TextDocumentPositionParams {
                text_document: jerry_lsp::lsp_types::TextDocumentIdentifier { uri: uri(target) },
                position: jerry_lsp::lsp_types::Position {
                    line: 3,
                    character: 8,
                },
    ...
    fn completion_params(target: &str) -> jerry_lsp::lsp_types::CompletionParams {
        jerry_lsp::lsp_types::CompletionParams {
            text_document_position: jerry_lsp::lsp_types::TextDocumentPositionParams {
                text_document: jerry_lsp::lsp_types::TextDocumentIdentifier { uri: uri(target) },
                position: jerry_lsp::lsp_types::Position {
                    line: 5,
    ...
    fn goto_definition_falls_back_to_the_companion_on_an_empty_array_answer() {
        ...
        let single = LspConnection::Single(primary.clone());
        let solo = single
            .request::<jerry_lsp::lsp_types::request::GotoDefinition>(
        ...
        assert!(
            matches!(
                solo,
                Some(jerry_lsp::lsp_types::GotoDefinitionResponse::Array(ref locations))
        ...
        let jerry_lsp::lsp_types::GotoDefinitionResponse::Array(locations) = answer else {
    ...
    fn completion_falls_back_to_the_companion_on_an_empty_completion_list() {
        ...
        let single = LspConnection::Single(primary.clone());
        let solo = single
            .request::<jerry_lsp::lsp_types::request::Completion>(
        ...
        assert!(
            matches!(
                solo,
                Some(jerry_lsp::lsp_types::CompletionResponse::List(ref list))
        ...
        let jerry_lsp::lsp_types::CompletionResponse::Array(items) = answer else {
        ...
        app.update(cx, |app, _cx| {
            app.completions = Some(CompletionsEntry {
                ...
                status: CompletionsStatus::Ready {
                    items: vec![jerry_lsp::lsp_types::CompletionItem {
    ...
    fn an_item_whose_resolve_was_cancelled_by_moving_on_is_resolved_again_on_return(
        ...
        let bare = |label: &str| jerry_lsp::lsp_types::CompletionItem {
    ...
    fn a_request_outside_the_fallback_list_never_consults_the_companion() {
        ...
        let params = jerry_lsp::lsp_types::ReferenceParams {
            text_document_position: jerry_lsp::lsp_types::TextDocumentPositionParams {
                text_document: jerry_lsp::lsp_types::TextDocumentIdentifier {
            ...
            work_done_progress_params: Default::default(),
            partial_result_params: Default::default(),
            context: jerry_lsp::lsp_types::ReferenceContext {
                include_declaration: false,
            },
        };
        let started = Instant::now();
        let result = connection
            .request::<jerry_lsp::lsp_types::request::References>(params, Duration::from_secs(10))
            .expect("a real response");
        assert_eq!(
            result,
            Some(Vec::new()),
    ...
    fn wait_until_dead(client: &jerry_lsp::LspClient) {
        assert!(
    ...
    async fn a_restart_frees_the_key_so_a_fresh_spawn_is_really_attempted(cx: &mut TestAppContext) {
        ...
        app.update(cx, |app, cx| {
            app.spawn_lsp_client(
                ...
                root.clone(),
                || {
                    Ok(jerry_lsp::ServerSpawnConfig {
...
/// `dispatch_did_open` -> `render_file_view`) rather than by calling `jerry_lsp` directly and
/// bypassing `AdeApp`.
#[cfg(test)]
mod lsp_diagnostics_wiring_tests {
    ...
    /// `jerry_lsp::client::tests::write_scratch_project` - kept as its own small copy here
    ...
    /// real `publishDiagnostics` notification arrives on `jerry_lsp`'s own raw OS reader thread,
    ...
    fn a_real_typescript_diagnostic_reaches_file_view_diagnostics_through_the_real_app_code_path(
        ...
        // See `jerry_lsp::client::tests::write_scratch_ts_project`'s own docs for why a real,
    ...
    fn a_transient_pull_failure_is_retried_not_treated_as_a_permanent_stall(
        ...
        let file_uri = jerry_lsp::LspClient::uri_for_path(&absolute).expect("real uri");
