...
//! published by `rust-analyzer` via `jerry_lsp::LspClient::diagnostics_for`) into per-line,
...
//! scope here - `jerry_lsp::LspClient`'s generic `request`/`notify` methods are equally usable
...
/// this app's own type so `crate::root`'s rendering code doesn't need to depend on `jerry_lsp`
