...
use jerry_git::diff::DiffFile;
use jerry_git::merge::ConflictHunk;
...
/// Cap on how many hunk lines a single file's diff renders, independent of `jerry_git::diff`'s
...
pub struct AdeApp {
    ...
    /// Every worktree of [`Self::focused_repo`], as read by `jerry_git::list_worktrees` -
    ...
    /// Every repository this app has open and its own real connection to that repository's
    /// session host - `crate::host`'s own module docs. A repository dispatch answers `NEEDS_HOST`
    /// until `Self::open_repo_host` has resolved a connection for it.
    pub(crate) hosts: crate::host::Hosts,
    ...
    pub(crate) uncommitted_diff: sections::ScopeLoad<jerry_git::diff::WorktreeDiff>,
    ...
    pub(crate) branch_commits: sections::ScopeLoad<jerry_git::diff::BranchCommits>,
    ...
    /// TextField` undo and from `jerry_git::undo`'s git-level commit/discard undo. GitHub issue
    ...
    /// real git index (`jerry_git::stage::staged_paths`) inside `Self::load_diff`'s own background
    ...
    /// unstaged, or untracked (`jerry_git::stage::dirty_paths`, a real `git status --porcelain`
    ...
    /// `jerry_git::run_drift::commits_since_each` per worktree on the background executor.
    ...
    /// The in-flight `jerry_git::review::snapshot_worktree_tree` behind each fresh agent's
    /// baseline capture, keyed by agent.
    pub(crate) _review_baseline_tasks: HashMap<AgentId, Task<()>>,
    /// The in-flight `jerry_git::review::diff_against_tree` load behind the review tab.
    ...
    /// In-flight `jerry_git::review::delete_ref` calls releasing closed agents' baseline refs,
    ...
    /// The in-flight `jerry_git::graph::build_graph` background load, one slot - a fresh load
    /// supersedes an older one still running, mirroring [`Self::_load_diff_task`].
    pub(crate) _load_graph_task: Option<Task<()>>,
    /// The in-flight `jerry_git::graph::commit_changed_files` background load behind the Commit
    ...
    /// Real, cached `jerry_git::blame::blame_file` results (GitHub issue #29), keyed by absolute
    ...
    /// Real, full commit-message bodies (`jerry_git::blame::commit_message`), keyed by commit sha
    ...
    /// Real `jerry_git::diff::AheadBehind` counts per worktree/agent cwd, from the same
    /// periodic refresh as [`Self::diff_cache`] - the status bar's `↑2 ↓0` indicator for the
    /// active agent's worktree.
    pub(crate) ahead_behind_cache: HashMap<PathBuf, jerry_git::diff::AheadBehind>,
    ...
    /// `jerry_pty::resolve_on_path`), computed off the foreground thread and cached here (see
    ...
    /// the (rare, honestly-reported) case `jerry_git::git_common_dir`/`notify::recommended_watcher`
    ...
    /// (`jerry_git::merge::write_resolved_file`) - a [`TaskPool`], not a single slot, since
    ...
    /// A `jerry_lsp::LspClient` per `(repository root, server binary)` pair - widened from a
    ...
    /// reordering [`jerry_lsp::LspClient::pull_diagnostics`]'s own version guard protects against
    ...
    /// A real, per-absolute-path cache of [`jerry_lsp::LspClient::uri_for_path`]'s own result
    ...
    /// identical rule already followed for `jerry_lsp::LspClient::diagnostics_for`) is that such a
    ...
    pub(crate) lsp_uri_cache: HashMap<PathBuf, jerry_lsp::lsp_types::Uri>,
    ...
    pub(crate) completions_resolved_items:
        std::collections::HashMap<usize, jerry_lsp::lsp_types::CompletionItem>,
    ...
    /// Every in-flight `jerry_lsp::LspClient::spawn`/`did_open` background task - a [`TaskPool`]
...
impl AdeApp {
    ...
    pub(crate) fn add_repo(&mut self, path: PathBuf, cx: &mut Context<Self>) -> RepoId {
        ...
        self.open_repo_host(self.repos[self.repos.len() - 1].path.clone(), cx);
        cx.notify();
        id
    }
...
mod repo_list_tests {
    ...
    /// in this module use) has no worktrees `jerry_git::list_worktrees_porcelain` can report at
    ...
    /// (`jerry_git::git_common_dir` failing), so a bare tempdir would make this test pass "by
