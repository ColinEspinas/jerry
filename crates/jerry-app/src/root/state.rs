...
impl AdeApp {
    ...
    fn load_worktrees_with_intent(&mut self, intent: WorktreeLoadIntent, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move { jerry_git::list_worktrees_porcelain(&repo_path) })
                .await;
    ...
    /// A real, one-shot `jerry_git::list_worktrees_porcelain` fetch for a single repo, writing the
    ...
    pub(crate) fn load_repo_worktrees(&mut self, repo_id: RepoId, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move { jerry_git::list_worktrees_porcelain(&path) })
                .await;
                    ...
                    let fetches: Vec<_> = paths
                        ...
                        .map(|(id, path)| {
                            cx.background_executor().spawn(async move {
                                (id, jerry_git::list_worktrees_porcelain(&path))
                            })
                        })
                        .collect();
    ...
    pub(crate) fn start_file_tree_watch(&mut self, cx: &mut Context<Self>) {
        ...
        if self.settings_path.is_none() || jerry_git::git_common_dir(&root).is_err() {
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let mut last_poll_fingerprint: Option<jerry_git::diff::ChangesFingerprint> = None;
            loop {
                ...
                let mut pending_fingerprint: Option<Option<jerry_git::diff::ChangesFingerprint>> =
                    None;
                let refresh_diff = if watcher_fired || !watcher_armed {
                    ...
                    let fingerprint = cx
                        .background_executor()
                        .spawn(async move {
                            jerry_git::diff::changes_fingerprint(&fingerprint_root).ok()
                        })
                        .await;
    ...
    pub(crate) fn reset_repo_scoped_state(
        ...
        // (`jerry_git::graph::GraphScope::Worktrees`, driven by real worktree HEADs) are real facts
...
/// `jerry_git`) - the panel really does converge to whatever `git` itself now reports, and
...
mod load_worktrees_integration_tests {
    ...
    fn a_real_worktree_remove_disappears_after_a_refresh(cx: &mut TestAppContext) {
        ...
        jerry_git::remove_worktree(repo.path(), &feature, false).expect("remove_worktree");
