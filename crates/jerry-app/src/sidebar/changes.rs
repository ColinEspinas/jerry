...
//! the mapping from `jerry_git::diff` data to which colours/labels/counts a row or fold marker
...
use crate::theme;
use jerry_git::diff::{DiffFile, DiffHunk, DiffLineKind, FileChangeStatus};

/// Added/removed line counts for one file, counted directly from its hunks.
/// `jerry_git::diff::DiffFile` has no separate stored counter for this, so it's recomputed here
...
pub fn stat_bar_segments(add: u32, del: u32) -> [StatSegment; STAT_BAR_LEN] {
    ...
    // Widened to `u64`: `add`/`del` are bounded today by `jerry_git::diff`'s
...
/// branch (that is why `jerry_git::diff::diff_against_base` listed it at all), but a real commit on
...
/// `jerry_git::diff` paths are repo-relative (stripped of the `a/`/`b/` prefixes `git diff`
...
/// `jerry_git::diff::DiffFile::old_path` is documented as "only set for renames", so this is a
...
/// new-file range. `jerry_git::diff::DiffHunk` only re-exposes the header's original text (the
/// parser that produced it is private to `jerry-git`), so the fold-marker treatment below needs
...
pub fn parse_hunk_new_range(header: &str) -> Option<(usize, usize)> {
    ...
    let count: usize = match parts.next() {
        ...
        // diff format (the same shorthand `jerry_git::diff`'s own private parser documents).
        None => 1,
    };
    Some((start, count))
}
...
mod diff_stat_tests {
    ...
    use jerry_git::diff::{DiffFile, DiffHunk, DiffLine, DiffLineKind, FileChangeStatus};
...
mod status_letter_tests {
    ...
    use crate::theme;
    use jerry_git::diff::FileChangeStatus;
...
mod hunk_header_tests {
    ...
    use jerry_git::diff::{DiffHunk, DiffLine, DiffLineKind};
...
mod row_label_tests {
    ...
    use jerry_git::diff::{DiffFile, FileChangeStatus};
    use std::path::{Path, PathBuf};
    ...
    /// or one identical to the current path (defensive: `jerry_git::diff` shouldn't produce it, but
