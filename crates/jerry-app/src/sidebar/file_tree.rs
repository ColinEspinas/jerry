...
/// worktree contain" (`jerry_git::worktree_files`, decisions.md §6) - tracked plus
...
fn build_worktree_file_tree_inner(
    ...
    let list = match jerry_git::worktree_files::list_worktree_files(root) {
        Ok(list) => list,
    ...
    let submodules = jerry_git::worktree_files::list_submodule_paths(root).unwrap_or_default();
...
fn from_git_listing(
    root: &Path,
    list: &jerry_git::worktree_files::WorktreeFileList,
) -> FileTreeListing {
...
mod git_listing_tree_tests {
    ...
    use jerry_git::worktree_files::WorktreeFileList;
    use std::fs;
    use std::path::Path;
