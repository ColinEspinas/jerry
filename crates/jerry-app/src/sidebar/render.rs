...
impl AdeApp {
    ...
    pub(in crate::sidebar) fn toggle_staged(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                ...
                .spawn(async move {
                    if should_stage {
                        jerry_git::stage::stage_path(&worktree_path, &git_path)
                    } else {
                        jerry_git::stage::unstage_path(&worktree_path, &git_path)
                    }
                })
                .await;
    ...
    /// paths>` + `git commit` (`jerry_git::undo::commit_paths`) on [`Self::diff_root`] (the
    ...
    /// **not**
    /// generally drop the just-committed files out of the Changes list: `jerry_git::diff`'s own
    ...
    pub(in crate::sidebar) fn commit_staged_files(&mut self, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    jerry_git::undo::commit_paths(&commit_worktree_path, &commit_paths, &message)
                })
                .await;
            let _ = this.update(cx, |this, cx| {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result: Result<String, String> = cx
                ...
                .spawn(async move {
                    match action {
                        CommitMenuAction::CommitAndPush => {
                            jerry_git::undo::commit_paths(&run_path, &staged, &message)
                            ...
                            jerry_git::remote::push_branch(
                                &run_path,
                                &branch,
                                jerry_git::remote::PushForce::None,
                            )
                            .map_err(|err| err.to_string())?;
                            Ok(format!(
                        ...
                        CommitMenuAction::CommitAllFiles => {
                            jerry_git::undo::commit_all_changes(&run_path, &message)
                                .map_err(|err| err.to_string())?;
                            Ok("committed every uncommitted file".to_string())
                        }
                        CommitMenuAction::AmendLastCommit => {
                            jerry_git::undo::amend_head_with_paths(&run_path, &staged)
                                .map_err(|err| err.to_string())?;
                            Ok(format!(
                        ...
                        CommitMenuAction::StashStaged => {
                            jerry_git::undo::stash_staged(&run_path, &format!("jerry: {message}"))
                                .map_err(|err| err.to_string())?;
                            Ok(format!(
                                "stashed {}",
                                plural::count(file_count, "file", None)
    ...
    /// exists, or the histories are unrelated - see `jerry_git::diff::DiffBase::NoBase`).
    ...
    fn changes_section_body(
        ...
        match section {
            ...
            ChangesSection::Uncommitted => {
                ...
                if diff.truncated {
                    body.push(warn(
                        "diff truncated: this checkout's real changes exceeded jerry_git::diff's \
            ...
            ChangesSection::Commits => {
                ...
                if commits.truncated {
                    body.push(warn(
                        "this branch has more commits than jerry_git::diff loads at once, so the \
            ...
            ChangesSection::AgainstMain => {
                ...
                if diff.truncated {
                    body.push(warn(
                        "diff truncated: this branch's real changes exceeded jerry_git::diff's own \
    ...
    /// [`Self::render_changes_runs_section`], with its own `max-height:170px` overflow rather
    /// than sharing this scroller.
    ...
    fn render_commit_row(&self, commit: &jerry_git::diff::BranchCommit) -> impl IntoElement {
    ...
    /// `jerry_git::stage::discard_path` for one file, on the background executor like every other
    ...
    pub(in crate::sidebar) fn discard_change_row(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move { jerry_git::stage::discard_path(&worktree_path, &git_path) })
                .await;
            let _ = this.update(cx, |this, cx| {
                match result {
                    Err(err) => {
...
/// `jerry_git` call - see [`AdeApp::run_commit_menu_action`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CommitMenuAction {
    /// `jerry_git::undo::commit_paths` then `jerry_git::remote::push_branch`.
    CommitAndPush,
    /// `jerry_git::undo::commit_all_changes` - `git add -A` and commit, not just the staged subset.
    CommitAllFiles,
    /// `jerry_git::undo::amend_head_with_paths` - folds the staged paths into the tip, keeping its
    /// message.
    AmendLastCommit,
    /// `jerry_git::undo::stash_staged` - `git stash push --staged`, leaving unstaged work alone.
    StashStaged,
}
...
/// `V mark seen`, `⌥click filter by author`, rendered through the same hint-row template as
/// every other footer, which is purely a loop over keycap+label pairs.
...
mod virtualization_tests {
    ...
    fn a_changes_row_far_below_the_viewport_is_never_painted(cx: &mut TestAppContext) {
        ...
        // A real feature branch, so this hits `jerry_git::diff::DiffBase::Diff` against a real
...
mod commit_composer_tests {
    ...
    /// establishes, so this hits `jerry_git::diff::DiffBase::Diff` against a real merge-base
    ...
    /// genuinely uncommitted edit** (`dirty.txt`). `jerry_git::diff::diff_against_base` diffs
    ...
    fn clicking_the_primary_button_reaches_the_real_commit_staged_files_action(
        ...
        assert_eq!(
            ...
            "a real click on the wired primary button must create exactly one real git commit \
             via jerry_git::undo::commit_paths"
        );
        assert!(
        ...
        assert_eq!(
            ...
            "the diff must be reloaded for real after the commit - both files still genuinely \
             differ from the merge-base with main (jerry_git::diff diffs the working tree against \
    ...
    fn the_changes_footers_hints_only_advertise_really_registered_bindings() {
        ...
        for (binding, spec) in [(stage, CHANGES_STAGE_SPEC), (seen, CHANGES_SEEN_SPEC)] {
            ...
            assert!(
                ...
                "{spec:?} is printed bare in the Changes footer, so a binding that silently \
                 gained a modifier would leave the keycap advertising a keystroke nobody can \
                 trigger"
            );
        }
