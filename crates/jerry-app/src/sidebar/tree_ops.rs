...
impl AdeApp {
    ...
    fn delete_path_with_real_undo(
        ...
        let mechanism =
            file_ops::resolve_delete_mechanism(std::env::consts::OS, &path, |program| {
                jerry_pty::resolve_on_path(program).is_some()
            });
                    ...
                    async move {
                        ...
                        match mechanism {
                            DeleteMechanism::Trash { program, args } => {
                                match jerry_pty::new_std_command(program).args(&args).status() {
    ...
    pub(in crate::sidebar) fn rename_open_paths(
        ...
        // further down: `staged_files` is keyed by `jerry_git::diff::DiffFile::path`
