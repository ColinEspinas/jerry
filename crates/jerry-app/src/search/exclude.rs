...
/// `./target` and `crates/jerry-app/target`, the same way the panel's own `*.lock` exclude field
...
mod tests {
    ...
    fn a_nested_default_excluded_directory_is_pruned_at_any_depth() {
        let dir = tempfile::tempdir().expect("tempdir");
        let root = dir.path();
        write(root, "crates/jerry-app/src/lib.rs", "fn main() {}\n");
        write(root, "crates/jerry-app/target/debug/x.o", "junk");

        let (files, _truncated) = collect_files_excluding(root, &default_exclude_list(), 20_000);
        assert_eq!(relatives(&files), vec!["crates/jerry-app/src/lib.rs"]);
    }
