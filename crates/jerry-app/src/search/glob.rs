//! The pure glob matcher behind the search panel's `include` / `exclude` path filters - a funnel
//! for path filters, behind `⋯`: `src/**, tests/**` / `target/**, *.lock`.
...
mod tests {
    ...
    fn a_pattern_with_no_slash_matches_the_basename_at_any_depth() {
        ...
        assert!(matches("*.lock", "crates/jerry-app/Cargo.lock"));
        assert!(!matches("*.lock", "crates/jerry-app/Cargo.toml"));
    }
