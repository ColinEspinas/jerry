...
fn main() -> ExitCode {
    ...
    #[cfg(windows)]
    jerry_app::job_object::adopt_this_process();
...
/// (`jerry_app::settings::store::AppearanceSettings::display_scale_override`).
#[cfg(any(target_os = "linux", target_os = "freebsd"))]
fn apply_display_scale_override() {
    let settings = jerry_app::settings::store::Settings::load_or_init();
    let existing = env::var(jerry_app::GPUI_X11_SCALE_FACTOR_ENV).ok();
    let Some(value) = jerry_app::x11_scale_factor_env_value(&settings, existing.as_deref()) else {
        return;
    };
    ...
    log::info!(
        "forcing {}={value} from appearance.display_scale_override (X11 sessions only; \
         restart-scoped)",
        jerry_app::GPUI_X11_SCALE_FACTOR_ENV
    );
    ...
    // the very top of `main`, before `jerry_app::run` starts GPUI and before this process has spawned
    // any thread of its own - so there is no other thread in existence to race. Every other place
    // that needed a custom environment (see `jerry_pty::resolve_in_path_var` and
    // `jerry_lsp::client`'s injected resolvers) passes the value as an argument specifically to
    ...
    #[allow(unsafe_code)]
    unsafe {
        env::set_var(jerry_app::GPUI_X11_SCALE_FACTOR_ENV, value)
    };
}
...
/// [`jerry_pty::resolve_login_shell_path`] is a no-op unless the inherited PATH actually looks
/// like that minimal case, so this is safe to call unconditionally on every unix launch.
#[cfg(unix)]
fn apply_login_shell_path() {
    let inherited = env::var_os("PATH").unwrap_or_default();
    let Some(resolved) = jerry_pty::resolve_login_shell_path(&inherited) else {
        return;
    };
    ...
    // of `main`, before `jerry_app::run` starts GPUI and before this process has spawned any thread of
    ...
    // `jerry_pty::resolve_on_path`'s callers) is deliberate, not an oversight: every agent PTY
    // `jerry_pty::spawn` launches inherits the process environment, so mutating it here is what
