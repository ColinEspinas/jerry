...
impl AdeApp {
    ...
    /// Captures agent `id`'s review baseline: a real `jerry_git::review::snapshot_worktree_tree` of
    ...
    pub(crate) fn capture_review_baseline(&mut self, id: AgentId, cx: &mut Context<Self>) {
        ...
        let ref_name = jerry_git::review::baseline_ref_name(&key);
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let result = cx
                ...
                .spawn({
                    ...
                    async move {
                        let snapshot = jerry_git::review::snapshot_worktree_tree(&worktree)?;
                        ...
                        jerry_git::review::anchor_tree(
                            &worktree,
                            &snapshot_ref_name,
                            &snapshot.tree_id,
                        )?;
                        Ok::<jerry_git::review::WorktreeSnapshot, jerry_git::Error>(snapshot)
                    }
                })
                .await;
    ...
    pub(crate) fn load_agent_review(&mut self, id: AgentId, cx: &mut Context<Self>) {
        ...
        // would be a lie. See `jerry_git::review::diff_against_tree`'s own docs.
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    jerry_git::review::diff_against_tree(&worktree, &tree_id, untracked, label)
                })
                .await;
            let _ = this.update(cx, |this, cx| {
    ...
    pub(crate) fn release_review_baseline(&mut self, id: AgentId, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move { jerry_git::review::delete_ref(&worktree, &ref_name) })
                .await;
            if let Err(err) = result {
    ...
    pub(crate) fn review_measure_targets(
        ...
        PathBuf,
        String,
        jerry_git::review::UntrackedCoverage,
    )> {
        self.agents
            .iter()
            .filter_map(|agent| {
