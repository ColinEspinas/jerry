...
impl AdeApp {
    ...
    pub(crate) fn open_review_file_detail(&self) -> Option<&jerry_git::diff::DiffFile> {
    ...
    pub(crate) fn render_review_view(&mut self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let files: Vec<jerry_git::diff::DiffFile> = review
            .diff()
...
/// real agents spawned into it, real `jerry_git::review` snapshots, and the real single-agent gate.
#[cfg(test)]
mod review_flow_tests {
    ...
    fn spawning_an_agent_captures_a_real_anchored_baseline_without_disturbing_git(
        ...
        assert!(
            ref_name.starts_with(jerry_git::review::REVIEW_REF_PREFIX),
