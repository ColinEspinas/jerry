...
use jerry_git::diff::WorktreeDiff;
use jerry_git::review::UntrackedCoverage;
...
pub struct ReviewBaseline {
    /// The real hex tree id from `jerry_git::review::snapshot_worktree_tree`.
    pub tree_id: String,
    /// The `refs/jerry/review/*` ref anchoring [`Self::tree_id`] against `git gc` -
    /// `jerry_git::review::baseline_ref_name`'s output for this agent's own key.
    pub ref_name: String,
    ...
    /// (`jerry_git::review::MAX_UNTRACKED_SNAPSHOT_BYTES`).
...
pub enum ReviewLoadState {
    ...
    /// advanced) - deliberately not eager, since `jerry_git::review::diff_against_tree` spawns a
    ...
    Loaded(WorktreeDiff),
    /// A real error message from the underlying `jerry_git` call, surfaced rather than swallowed
...
pub struct AgentReview {
    ...
    /// rail's own status-poll tick (`jerry_git::review::changed_paths_against_tree`: one
