//! `jerry-app`: the ADE desktop application shell.
//!
//! A three-pane GPUI window: a left sidebar listing the target repository's real git
//! worktrees (via `jerry-git`) with agent/tab controls for spawning agent CLIs or shells
//! into them, a tabbed center pane of real terminal agents (via `jerry-pty` +
...
/// `crates/jerry-app/resources/linux/jerry.desktop.in`). Decoded once, not per window: `default_window_options`
