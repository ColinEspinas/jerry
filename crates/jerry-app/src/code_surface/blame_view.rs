...
impl AdeApp {
    ...
    /// Dispatches one real, off-thread `jerry_git::blame::blame_file` call for `absolute_path`,
        ...
        let task = cx.spawn({
            ...
            async move |this, cx| {
                let result = cx
                    ...
                    .spawn({
                        ...
                        async move {
                            ...
                            let len = metadata.map(|meta| meta.len()).unwrap_or(0);
                            let outcome =
                                jerry_git::blame::blame_file(&worktree_root, &relative_path);
                            (outcome, mtime, len)
                        }
                    })
                    .await;
                ...
                let _ = this.update(cx, |this, cx| {
                    match outcome {
                        Ok(jerry_git::blame::BlameOutcome::Blame(blame)) => {
                        ...
                        Ok(
                            jerry_git::blame::BlameOutcome::NotARepo
                            | jerry_git::blame::BlameOutcome::NotTracked,
                        ) => {
    ...
    pub(in crate::code_surface) fn current_line_blame(
        ...
        buffer_dirty: bool,
    ) -> Option<&jerry_git::blame::BlameLine> {
    ...
    /// sha (`jerry_git::blame::commit_message` itself already refuses to look that up - checked
        ...
        let task = cx.spawn({
            ...
            async move |this, cx| {
                let result = cx
                    ...
                    .spawn({
                        let sha = sha.clone();
                        async move { jerry_git::blame::commit_message(&worktree_root, &sha) }
                    })
                    .await;
                let _ = this.update(cx, |this, cx| {
                    match result {
