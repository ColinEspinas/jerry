...
//! #14 "Editor polish"): turning a [`jerry_git::blame::BlameLine`] into the short label shown
...
use jerry_git::blame::BlameLine;

/// The all-zero sha `git blame` uses for a line whose content hasn't been committed yet -
/// re-derived here rather than imported, since [`jerry_git::blame::BlameLine::is_uncommitted`]
...
/// `line.sha` (`jerry_git::blame::commit_message`); without it, the tooltip falls back to the
