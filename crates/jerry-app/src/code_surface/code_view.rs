...
use crate::theme;
use jerry_git::diff::{DiffFile, DiffLineKind};

/// Cap on how many bytes of a file [`load_file`] will actually read and highlight, matching
/// `jerry_git::diff`'s `MAX_DIFF_OUTPUT_BYTES` (`crates/jerry-git/src/diff.rs:87`) so both caps stay
...
/// up - bounded the same way `jerry_git::diff::MAX_HUNK_LINES_PER_FILE`-style caps are, so a huge
...
mod tests {
    use super::*;
    use jerry_git::diff::{DiffHunk, DiffLine, FileChangeStatus};
