...
pub(crate) struct ChangesSnapshot {
    ...
    pub(crate) branch_commits: crate::sidebar::sections::ScopeLoad<jerry_git::diff::BranchCommits>,
...
pub(crate) struct HoverAnchor {
    ...
    pub(in crate::code_surface) position: jerry_lsp::lsp_types::Position,
}
...
/// The outcome of the most recent (or in-flight) `jerry_git::blame::blame_file` call for one
...
pub(crate) enum BlameLoadState {
    ...
    /// expected outcome per `jerry_git::blame`'s own "graceful absence" contract, never shown as
    /// an error.
    Unavailable,
...
pub(crate) struct BlameCacheEntry {
    ...
    pub(in crate::code_surface) blame: jerry_git::blame::FileBlame,
}

/// The outcome of the most recent (or in-flight) `jerry_git::blame::commit_message` call for one
