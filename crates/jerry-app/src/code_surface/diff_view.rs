...
/// really was cut short (either by that cap here or by `jerry_git::diff`'s own load-time cap,
/// `DiffFile::truncated`).
...
impl AdeApp {
    ...
    /// `jerry_git::diff` has no lazy per-file hunk-loading state, since every non-binary changed
    ...
    /// of `jerry_git::diff`'s own load-time cap.
    pub(crate) fn render_diff_file_detail(
        ...
        let list = uniform_list(
            ...
            cx.processor(move |this: &mut Self, range: Range<usize>, _window, cx| {
                ...
                // method was handed, and cloning one (up to `jerry_git::diff`'s own per-file line
...
/// short - by [`MAX_RENDERED_DIFF_LINES_PER_FILE`] here, or by `jerry_git::diff`'s own load-time
...
mod diff_render_tests {
    ...
    fn sample_diff_file(path: &str) -> DiffFile {
        DiffFile {
            ...
            old_path: None,
            status: jerry_git::diff::FileChangeStatus::Modified,
            is_binary: false,
            hunks: vec![jerry_git::diff::DiffHunk {
                header: "@@ -1,1 +1,1 @@".to_string(),
                lines: vec![jerry_git::diff::DiffLine {
...
mod diff_row_plan_tests {
    ...
    fn line(kind: DiffLineKind, content: &str) -> jerry_git::diff::DiffLine {
        jerry_git::diff::DiffLine {
            kind,
            content: content.to_string(),
        }
    }

    fn file_with(hunks: Vec<jerry_git::diff::DiffHunk>, truncated: bool) -> DiffFile {
        DiffFile {
    ...
    fn the_row_plan_stops_at_the_render_cap_and_ends_with_the_truncation_notice() {
        let lines: Vec<jerry_git::diff::DiffLine> = (0..MAX_RENDERED_DIFF_LINES_PER_FILE + 50)
            .map(|index| line(DiffLineKind::Added, &format!("line {index}")))
            .collect();
        let file = file_with(
            vec![jerry_git::diff::DiffHunk {
