...
mod editing_tests {
    ...
    /// (`crates/jerry-app/src/code_surface/tabs.rs`) for making sure the background load has actually
    ...
    /// would also report changes through, per `jerry_git::diff::diff_against_base`'s own docs),
    ...
    fn completions_keybindings_are_correctly_scoped_in_both_the_open_and_closed_state(
        ...
        let fake_item = |label: &str| jerry_lsp::lsp_types::CompletionItem {
    ...
    fn seed_completions(
        ...
        let items = labels
            .iter()
            .map(|label| jerry_lsp::lsp_types::CompletionItem {
                label: label.to_string(),
                ..Default::default()
            })
            .collect();
