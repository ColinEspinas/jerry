...
//! loads behind them (`jerry_git::diff` for a diff, `std::fs::read_to_string` +
...
impl AdeApp {
    ...
    /// the five real `jerry_git` queries (`diff_against_base`, `staged_paths`, `dirty_paths`,
            ...
            let (diff_result, staged_result, dirty_result, uncommitted_result, commits_result) = cx
                ...
                .spawn({
                    ...
                    async move {
                        ...
                        let diff_result = jerry_git::diff::diff_against_base(&root).map(|base| {
                        ...
                        let staged_result = jerry_git::stage::staged_paths(&root);
                        let dirty_result = jerry_git::stage::dirty_paths(&root);
                        ...
                        // `jerry_git::diff::diff_against_head`'s and
                        // `jerry_git::diff::commits_since_base`' own docs.
                        let uncommitted_result = jerry_git::diff::diff_against_head(&root);
                        let commits_result = jerry_git::diff::commits_since_base(&root);
                        (
                            diff_result,
                            staged_result,
                            dirty_result,
    ...
    /// itself is unborn (see [`jerry_git::diff::DiffBase::diff`]). A worktree on its default
