...
impl AdeApp {
    ...
    pub(in crate::code_surface) fn render_file_view(
        ...
        let lsp_status = if let Some(language_id) = language_id {
            ...
            let file_uri = jerry_lsp::LspClient::uri_for_path(&absolute_path).ok();
        ...
        // this same real suppression below: it comes from `jerry_git::diff`, a real diff against
...
";
    ...
    fn the_breadcrumb_counts_come_from_a_real_diagnostics_list() {
        let diagnostics = vec![
            severity_diagnostic(jerry_lsp::lsp_types::DiagnosticSeverity::ERROR),
            severity_diagnostic(jerry_lsp::lsp_types::DiagnosticSeverity::WARNING),
            severity_diagnostic(jerry_lsp::lsp_types::DiagnosticSeverity::WARNING),
            severity_diagnostic(jerry_lsp::lsp_types::DiagnosticSeverity::HINT),
        ];
    ...
    fn severity_diagnostic(
        severity: jerry_lsp::lsp_types::DiagnosticSeverity,
    ) -> jerry_lsp::lsp_types::Diagnostic {
        jerry_lsp::lsp_types::Diagnostic {
            range: jerry_lsp::lsp_types::Range {
                start: jerry_lsp::lsp_types::Position {
                    line: 0,
                    character: 0,
                },
                end: jerry_lsp::lsp_types::Position {
                    line: 0,
                    character: 4,
                },
            },
...
mod lsp_status_label_tests {
    ...
    fn a_failure_message_is_shown_as_is_because_it_already_names_its_own_server() {
        ...
        // `jerry_lsp::LspError` variant carries its own `server`), so it is also shown untouched.
