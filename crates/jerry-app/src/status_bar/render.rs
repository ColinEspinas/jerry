...
impl AdeApp {
    ...
    /// `jerry_git::diff::ahead_behind_against_base` refresh as [`Self::diff_cache`]). A detached
    ...
    pub(crate) fn build_resource_tree(&self, cx: &mut Context<Self>) -> ResourceTree {
        ...
        // whose `jerry_git::list_worktrees` fetch has not landed, a directory that is not a git
