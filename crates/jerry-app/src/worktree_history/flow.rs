...
impl AdeApp {
    ...
    /// it. Display-only: never consulted by a real `jerry_git::undo::*` call.
    ...
    /// The Review footer's `Keep all` action - a real `jerry_git::undo::commit_all_changes` on
    ...
    pub(crate) fn keep_all_changes(&mut self, id: AgentId, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move { jerry_git::undo::commit_all_changes(&worktree_path, &message) })
                .await;
    ...
    /// `jerry_git::undo::discard_worktree` is called from, whether the confirmation came from the
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let result = cx
                ...
                .spawn(async move {
                    ...
                    jerry_git::undo::discard_worktree(&repo_path, &worktree_path)
                })
                .await;
            ...
            let _ = this.update_in(cx, |this, window, cx| {
                ...
                match result {
                    Ok(snapshot) => {
                        // `jerry_git::undo::DiscardSnapshot::had_ignored_content` is a real,
...
mod worktree_history_regression_tests {
    ...
    fn keep_all_changes_commits_a_dirty_worktree(cx: &mut TestAppContext) {
        ...
        assert!(
            !jerry_git::is_dirty(&feature).expect("is_dirty"),
    ...
    fn a_second_keep_all_changes_while_the_first_is_in_flight_does_not_race_it(
        ...
        assert!(!app.read_with(cx, |app, _| app.worktree_history_op_in_flight.is_some()));
        assert!(
            !jerry_git::is_dirty(&feature_a).expect("is_dirty a"),
            "feature-a's real commit must have gone through"
        );
        assert!(
            jerry_git::is_dirty(&feature_b).expect("is_dirty b"),
