...
/// Runtime kill switch, for anything linking `jerry-app` across a crate boundary that `cfg(test)` can
/// not reach.
...
mod update_check_gate_tests {
    ...
    fn this_very_test_binary_may_not_check_for_updates() {
        assert!(
            !update_check_enabled(),
            "the compiled-in gate must hold for the jerry-app crate's own test targets"
        );
    }
}
