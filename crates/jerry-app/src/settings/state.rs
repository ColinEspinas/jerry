...
pub struct AgentRow {
    ...
    /// installed - never a guess. The search (`jerry_pty::resolve_on_path`) checks file
...
/// `jerry_pty::resolve_on_path` - see [`AgentRow::resolved_path`] for the one disclosed gap in
...
pub enum WorktreeDotStatus {
    ...
    Prunable,
    /// `jerry_git::is_dirty` failed for this path - genuinely unknown, not a guess.
    Unknown,
}
...
/// Resolves the configured shell the same two ways `jerry_pty::spawn` itself will (GitHub issue
...
pub struct ShellSuggestion {
    ...
    /// means exactly one program: `jerry_pty::spawn` runs an absolute path verbatim, with no
...
/// `jerry_pty::resolve_on_path`.
pub fn detect_installed_shells() -> Vec<ShellSuggestion> {
    #[cfg(unix)]
    {
        unix_shell_suggestions(std::path::Path::new(ETC_SHELLS), jerry_pty::resolve_on_path)
    }
    #[cfg(windows)]
    {
        windows_shell_suggestions(std::env::var_os("COMSPEC"), jerry_pty::resolve_on_path)
    }
}
...
mod tests {
    use super::*;
    use crate::rail::state::WorktreeNote;
    use jerry_git::diff::WorktreeMergeStatus;
    ...
    fn the_real_path_resolver_finds_a_real_shell() {
        let status = detect_shell_status(Some("sh"), jerry_pty::resolve_on_path);
        match status {
