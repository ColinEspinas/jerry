...
impl AdeApp {
    ...
    pub(crate) fn load_agent_rows(&mut self, cx: &mut Context<Self>) {
        let task = cx.spawn(async move |this, cx| {
            let rows = cx
                .background_executor()
                .spawn(async move { settings::detect_agent_rows(jerry_pty::resolve_on_path) })
                .await;
    ...
    pub(crate) fn load_lsp_rows(&mut self, cx: &mut Context<Self>) {
        let task = cx.spawn(async move |this, cx| {
            let rows = cx
                .background_executor()
                .spawn(async move { settings::detect_lsp_rows(jerry_pty::resolve_on_path) })
                .await;
            let _ = this.update(cx, |this, cx| {
                this.lsp_rows = rows;
                cx.notify();
    ...
    /// `jerry_pty::resolve_on_path`). Called on every edit and when Settings opens - never from
